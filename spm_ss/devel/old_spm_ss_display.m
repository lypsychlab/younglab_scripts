...
elseif ischar(activationfiles),
    switch(lower(activationfiles)),
        ...
        case 'slice'
            ...
            mb=spm_read_vols(ma);
            [x,y]=ndgrid(1:ma.dim(1),1:ma.dim(2));
            ...
            idx=find(all(~thisplot,4));thisplot(idx)=mb(idx);thisplot(idx+numel(mb))=mb(idx);thisplot(idx+2*numel(mb))=mb(idx);
            
            hfig2=figure('name',[mfilename,' mosaic view'],'numbertitle','off','units','norm','position',[0,.3,1,.6],'color','k');
            h=[];hi=[];
            for n2=1:nplots,
                h(n2)=subplot(nplots2(1),nplots2(2),n2);
                slice=ceil(n2*ma.dim(3)/(nplots+1));
                mbt=permute(thisplot(:,:,slice,:),[1,2,4,3]); %reshape(spm_get_data(ma,pinv(ma.mat)*ma.mat*xyz'),ma.dim(1:2));
                hi(n2)=imagesc(max(0,min(1, permute(mbt(end:-1:1,end:-1:1,:),[2,1,3])) ));%flipud(fliplr(mbt')));
                axis equal; axis off;
            ...
            for nfiles=1:numel(data.roifiles),
                ...
                for n2=1:nplots,
                    ...
                    b=reshape(round(spm_get_data(a1,pinv(a1.mat)*ma.mat*xyz')),ma.dim(1:2));
                    axes(h(n2));
                    call=0;
                    ...
                    if data.intersectactivations,
                        mbt=permute(thisplot(:,:,slice,:),[1,2,4,3]); 
...
if ~isempty(data.plot_activation)&&~isempty(nfiles), 
    ...
    elseif all(~isnan(nfiles))&&numel(unique(nfiles))==1,
        nfile=unique(nfiles);
        emph=1;cmap=get(data.hfig,'colormap');thisplot=max(0,min(1, ind2rgb(round(1+((data.plot_activation{nfile}{nview}(:,:,1)>.05).*(1+emph*(size(cmap,1)-1)*data.plot_activation{nfile}{nview}(:,:,3)))),[zeros(1,3);flipud(cmap)]) ));
    else
        ntot=0;
...
%     else cmap=winter(numel(data.plot_rois_idx)+2);cmap=cmap(2:end-1,:);end
    cmap=get(data.hfig,'colormap');cmap=cmap(round(linspace(1,size(cmap,1),numel(data.plot_rois_idx)+2)),:);cmap=cmap(2:end-1,:);
...
if data.init_display,
    ...
    if any(temp(:)>0),
        hold on;
        [nill,hc]=contour(temp,.5:numel(data.plot_rois_idx)+.5,'k-');set(hc,'linecolor',.5*[1,1,1],'linewidth',2,'linestyle','-');
    ...
    if numel(mfiles)==1&&numel(data.thresholded)>=mfiles&&isempty(data.thresholded{mfiles})&&~isnan(data.range_activation{mfiles}),
        cmap0=[linspace(.85,1,64)',linspace(1,0,64)'];cmap=zeros([size(cmap0,1),1,3]);cmap(:,:,idxmfiles)=cmap0(:,1+zeros(1,numel(idxmfiles)));cmap(:,:,setdiff(1:3,idxmfiles))=cmap0(:,2+zeros(1,3-numel(idxmfiles)));axes('units','norm','position',[.925,.5,.025,.3]);image(cmap);set(gca,'ydir','normal'); axis off; hold on; ht=text([1,1],[1-.1*size(cmap,1),size(cmap,1)*1.1],{['0'],[num2str(data.range_activation{mfiles},'%0.2f')]});set(ht,'color',.85*[1,1,1],'horizontalalignment','center');
        data.handles{15}=gca;
