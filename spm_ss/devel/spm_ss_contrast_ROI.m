...
cd(ss.swd);
ss.VM=spm_vol(ss.PM);
frois=round(spm_read_vols(ss.VM));
nrois=max(frois(:));
    ...
    if overwrite||~isfield(ss,'evaluate')||numel(ss.evaluate)<Ic||isempty(ss.evaluate{Ic}),
        ...
        for nroi=1:nrois,
            ...
            if ~any(any(isnan(B),1),2) && ~any(any(isnan(E))) && dof-Nx>2,
                ...
                b=B;
                rss=E;
                [h,f,p,nill,stderr]=spm_ss_glm('evaluate',x,b,rss,ss.C(Ic).between,ss.C(Ic).within,dof,Nx,Nc0,Nh,Ns); % general linear model
                Hplane(:,:,nroi)=h;
                Fplane(nroi)=f;
                Pplane(nroi)=p;
                Eplane(:,:,nroi)=stderr;
        ...
        nh=1;for nh1=1:Nh(1),for nh2=1:Nh(2),z=nan+zeros(size(frois));for nroi=1:nrois,z(frois==nroi)=Hplane(nh1,nh2,nroi);end; spm_write_vol(VH(nh),z);nh=nh+1;end;end
        z=nan+zeros(size(frois));for nroi=1:nrois,z(frois==nroi)=Fplane(nroi);end; spm_write_vol(VF,z);
        z=nan+zeros(size(frois));for nroi=1:nrois,z(frois==nroi)=Pplane(nroi);end; spm_write_vol(VP,z);
        z=nan+zeros(size(frois));for nroi=1:nrois,z(frois==nroi)=DOFplane(nroi);end; spm_write_vol(VDOF,z);
        z=nan+zeros(size(frois));for nroi=1:nrois,z(frois==nroi)=PFDRplane(nroi);end; spm_write_vol(VPfdr,z);
        ...
        fprintf(fh,'ROI#,ROI size,average localizer mask size,inter-subject overlap,'); for nh1=1:Nh(1),for nh2=1:Nh(2),fprintf(fh,'h[%d;%d],',nh1,nh2);end; end; if prod(Nh)==1,fprintf(fh,'stderr,'); end;fprintf(fh,'T/F,dof,p,p-fdr\n');
        for nroi=1:nrois,
            fprintf(fh,'%d,%d,%d,%f,',nroi,round(ss.estimate.voxels(nroi)),round(ss.estimate.voxels(nroi)*ss.estimate.coverage(nroi)),ss.estimate.overlap(nroi)); for nh1=1:Nh(1),for nh2=1:Nh(2),fprintf(fh,'%f,',ss.evaluate{Ic}.con(nh1,nh2,nroi));end; end; if prod(Nh)==1,fprintf(fh,'%f,',ss.evaluate{Ic}.con(1,1,nroi)./ss.evaluate{Ic}.t(nroi));end;fprintf(fh,'%f,%f,%f,%f\n',ss.evaluate{Ic}.t(nroi),ss.evaluate{Ic}.dof(nroi),ss.evaluate{Ic}.p(nroi),ss.evaluate{Ic}.pfdr(nroi));
        end
        fclose(fh);
    end
end
