...
if ss.typen==2,
    ...
    a2=spm_vol(ss.PM2);
    b2=spm_read_vols(a2);
    b3=spm_ss_watershed(-b2,find(b2>=ss.overlap_thr));
...
if numel(XM)>1&&any(size(frois)~=size(XM)),error('mismatched volume dimensions between ROI and explicit mask images'); end
frois=XM.*frois;
nrois=max(frois(:));
...
if any(ss.VM(1).dim~=ss.VY(1).dim),error('mismatched volume dimensions in ROI image file'); end
