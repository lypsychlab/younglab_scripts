function [thr_p,thr_type,filenames]=spm_ss_threshold(option,spm_data,Ic,Ec)
% SPM_SS_THRESHOLD
...
switch(lower(option))
    ...
    case 'subject',
        ...
        if AnyAutomatic
            ...
            for nic2=1:size(Ic,2),
                ...
                if strcmpi(Threshold_type{nic2},'automatic')
                    for nic1=1:size(Ic,1),
                        ...
                        N=numel(Y);
                        [nill,idx]=sort(Y(:));
                        Q=zeros(N,1);Q(idx)=((1:N)'/N).*((1-Y(idx)).^2);
                        [nill,idxt]=max(Q);
            ...
            MaskFilenames{end+1}=spm_ss_createlocalizermask(spm_data.SPM,Ic,Ec,0,Threshold_type,Threshold_p);
        end
        for nexp=1:numel(spm_data.SPM),
            idx=find(Ec==nexp);
            spm_data.SPM{nexp}.xCon=spm_data.SPM{nexp}.xCon(Ic(:,idx));
            Ic(:,idx)=reshape(1:numel(spm_data.SPM{nexp}.xCon),[size(Ic,1),numel(idx)]);
        end
    ...
    case 'end',
        ...
        if nargout>1&&AnyAutomatic
            for np=1:numel(SPMfiles),
                ...
                MaskFilenames{np}=spm_ss_createlocalizermask(SPMfiles{np},ContrastIndexes{np},ExpIndexes{np},0,Threshold_type,Threshold_p);
            end
        end
        thr_p=Threshold_p;
