function maskfilename=spm_ss_createlocalizermask(SPM,Ic,Ec,overwrite,thr_type,thr,options)
...
if ~isempty(thr_type)&&ischar(thr_type)&&isempty(strmatch(thr_type,{'FDR','FWE','none','percentile-whole-brain','percentile-ROI-level'},'exact')),return;end
maskfilename={};
...
if isempty(thr_type),
    if isnumeric(posstr)&&isempty(findobj(0,'tag','Interactive')), spm('CreateIntWin'); end;
    str='FDR|FWE|none|percentile-whole-brain|percentile-ROI-level';
    thr_type={};
    if numel(thr)>1
    ...
    for nic1=1:size(Ic,1), % computes one separate thresholded volume per row and column of Ic (multiple columns are treated as separate contrasts)
        for nic2=1:size(Ic,2),
            ...
            if overwrite||isempty(dir(maskfilename{nic1,nic2})),
                Z=1;
                U={};
                if ~isempty(strmatch(thr_type{nic2},{'FDR','FWE','none','percentile-whole-brain','percentile-ROI-level'},'exact')),
                    %         try,
                    ...
                    switch(thr_type{nic2}),
                        ...
                        case 'percentile-whole-brain'
                            sb=sort(b(idx));
                            Y=double(b>=sb(max(1,min(numel(sb),ceil(numel(sb)*(1-thr(nic2)))))));
                ...
                if numel(Z)>1
                    ...
                    if any(strcmp(thr_type{nic2},'percentile-ROI-level')), Vo.dt(1)=spm_type('float32'); end
        ...
        if overwrite||isempty(dir(maskfilename{nic1})),
            ...
            if numel(Z)>1
                ...
                Vo=struct(...
                    ...
                    'mat',      a.mat,...
                    'descrip',  sprintf('SPM_SS LOCALIZER{%s}',cat(2,U{:})));
                if any(strcmp(thr_type,'percentile-ROI-level')), Vo.dt(1)=spm_type('float32'); end
                Vo=spm_write_vol(Vo,Z);
                cd(cwd0);
            end
        end
