...
% flags threshold-ROI masks
flag=nan(1,numel(ss.VN));
for n=1:numel(ss.VN)
    flagmatch=cellfun(@(x)str2double(x),regexp(ss.VN(n).descrip,'p=([.\d]+)\(percentile-ROI-level\)','tokens'));
    if numel(flagmatch)>1&&any(diff(flagmatch)), disp('warning: conjunction of different percentile-ROI-level thresholds not implemented. Using first threshold value'); end
    if ~isempty(flagmatch), flag(n)=flagmatch(1); end
end
...
for nroi=1:nrois,
    ...
    for nc=find(~isnan(flag))
        tN=N(nc,:);
        tN(tN==0|isnan(tN))=-inf;
        tNthreshold=sort(tN);
        tNthreshold=tNthreshold(max(1,min(numel(tN), ceil(numel(tN)*(1-flag(nc))))));
        N(nc,:)=double(tN>tNthreshold);
    end
