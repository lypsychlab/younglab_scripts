...
ss.PM=ss.ManualROIs;
ss.VM=spm_vol(ss.PM);
if numel(ss.VM)>1,error('multiple ROI image files not supported; use single ROI file with multiple ROI indexes instead'); end
...
% analysis
extname=['_',ss.type];
Nplane=nan+zeros([1,nrois]);Qplane=nan+zeros([ss.n,neffects,neffects,nrois]);
...
fprintf(fh,'\n');
fprintf(fh,'ROI#,ROI size,');
for ns=1:ss.n,for ne1=1:neffects,for ne2=ne1:neffects,fprintf(fh,'Subject#%d[%d;%d]',ns,ne1,ne2); if ne1<neffects||ne2<neffects||ns<ss.n, fprintf(fh,','); else fprintf(fh,'\n'); end; end; end; end
for nroi=1:nrois,
    fprintf(fh,'%d,%d,',nroi,round(ss.estimate.voxels(nroi)));
    for ns=1:ss.n, for ne1=1:neffects,for ne2=ne1:neffects,fprintf(fh,'%f',ss.estimate.overlap(ns,ne1,ne2,nroi)); if ne1<neffects||ne2<neffects||ns<ss.n, fprintf(fh,','); else fprintf(fh,'\n'); end; end; end; end
end
fclose(fh);
