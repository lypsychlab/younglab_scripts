...
elseif ischar(activationfiles),
    switch(lower(activationfiles)),
        ...
        case 'slice'
            ...
            idx=find(all(~thisplot,4));thisplot(idx)=mb(idx)/4;thisplot(idx+numel(mb))=mb(idx)/4;thisplot(idx+2*numel(mb))=mb(idx)/4;
...
if data.init_display,
    ...
    if any(temp(:)>0),
        hold on;
        [nill,hc]=contour(temp,.5:numel(data.plot_rois_idx)+.5,'k-');set(hc,'linecolor',.5*[1,1,1],'linewidth',1,'linestyle','-');
