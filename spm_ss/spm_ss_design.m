...
%    ss.Localizer_thr_type           cell array of multiple comparisons correction types for each first-level localizer mask ('FDR','FWE','none','percentile-whole-brain','percentile-ROI-level','automatic') (default 'FDR')
...
if ~ss.files_selectmanually&&(ss.askn>1||~isfield(ss,'Localizer_thr_type')||numel(ss.Localizer_thr_type)<size(ss.Localizer_contrasts,2)||~isfield(ss,'Localizer_thr_p')||numel(ss.Localizer_thr_p)<size(ss.Localizer_contrasts,2)),
    ...
    if ss.askn,
        ...
        for n1=1:size(ss.Localizer_contrasts,2)
            types={'FDR','FWE','none','automatic','percentile-whole-brain','percentile-ROI-level'};
            ...
            disp(str);
            sstype1{n1}=types{spm_input(str,posstr,'m','FDR|FWE|none|automatic|percentile-whole-brain|percentile-ROI-level',[],sstype)};posstr='+1';
