function younglab_MVPA_general(roin, experiments,partition_names,condition_names_all,savedirectory,roinum, groups, group_loc,parametric)
...
addpath /software/conn % from /software/gablab/conn at mit
...
Rraw_average = nanmean(nanmean(Rraw,5),4);
R_average = nanmean(nanmean(R,5),4);

Zraw_average = nanmean(nanmean(Zraw,5),4);
...
for ncondition1=1:size(Z,1),
    for ncondition2=[1:ncondition1-1,ncondition1+1:size(Z,1)],
        if parametric == 1
            acc_pairwise(ncondition1,ncondition2,:,:)= mean( Z(ncondition1,ncondition1,:,:,:)-Z(ncondition1,ncondition2,:,:,:), 5);  %changed from > to - for parametric
