function searchlight_all_twomatrix_roi(study,subj_tag,resdir,sub_nums,conditions,sph,b1,b2,roiname)
...
% - [searchlight images]: .nii files prepended with "RSA_searchlight", in results dir
	...
	for subj=1:length(subjIDs) %grabbing beta images
        ...
        behav_1 = sim2tril(behav_matrix);clear behav_matrix;
        load(fullfile(rootdir, study, 'behavioural' ,['behav_matrix_' b2 '.mat']));
		% behav_matrix=behav_matrix((c1+1):c2,(c1+1):c2);
        behav_2 = sim2tril(behav_matrix);clear behav_matrix;
...
%         disp([repmat([fullfile(rootdir,study,subjIDs{subj},'results/DIS_results_itemwise_normed/')],conditions,1) char(betafiles) repmat(',1',conditions,1)]);
        ...
        corrs(3)=weights(1); % constant

	    clear temp simmat behav_1 behav_2 weights predictors

 	    save(['bigmat_' roiname '_' b1 '_' b2 '.mat'], 'bigmat');
	    save (['corrs_' roiname '_' b1 '_' b2 '.mat'], 'corrs');
