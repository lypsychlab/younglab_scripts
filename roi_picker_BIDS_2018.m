...
if i>length(subjects) % just finished last subject
    ...
    try
        ...
        imgdir = dir(fullfile('/data/younglw/lab',study,'derivs/fmriprep',bids_id,'anat','*T1w*MNI*preproc.nii'));
        if length(imgdir)==0
            imgdir = dir(fullfile(subjects{i},'3danat','ws*.nii'));
        end
        evalin('base',['spm_sections(xSPM,findobj(spm_figure(''FindWin'',''Interactive''),''Tag'',''hReg''),''' fullfile('/data/younglw/lab',study,'derivs/fmriprep',bids_id,'anat',imgdir(1).name) ''');']);
    catch
