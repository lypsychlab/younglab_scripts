function searchlight_all_regress_six(study,subj_tag,resdir,sub_nums,conditions,sph,b1,b2,b3,b4,b5,b6,outtag)
...
% - [searchlight images]: .nii files prepended with "RSA_searchlight_regress", in results dir
	...
	for subj=1:length(subjIDs) %grabbing beta images
	    ...
	    corrs  = zeros(length(greymattermask2),7); 
	    for i = 1:length(greymattermask2)% for each voxel
	        ...
	        if length(goodrows) > 9 % if there are at least 10 good voxels in this sphere
	            ...
	            predictors = horzcat(ones(length(behav_1),1),behav_1,behav_2,behav_3,behav_4,behav_5,behav_6);
	            ...
	            corrs(i,6)=weights(7); % b5

                corrs(i,7)=weights(1); % constant

	        end
	    end
	    clear temp simmat behav_1 behav_2 behav_3 behav_4 behav_5 behav_6 weights predictors
        ...
        template.fname = ['RSA_searchlight_regress_' b6 outtag '.img']; spm_write_vol(template,corrmap_6);
	   	template.fname = ['RSA_searchlight_regress_const_' outtag '.img']; spm_write_vol(template,corrmap_7);
