function younglab_preproc_spatial_noslicetime (varargin)
%
    ...
    if prep_seq == 4 % now if a 4 remains, smooth
        ...
        % prep_seq=4 and wrrun when prep_seq>=6
         try
            ...
            fprintf ('Deleting wrf* and rf* files for subject %s\n',subjID);
            for dirs = 1:(length(number_boldDir)) % removing all wrrun* and rrun* files in each bold dir
...
try
    ...
    if realigned && ~normalised
        % Use spm_get to grab "rruns"
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, 'bold', char(func_runs(run))), 'rf*.img');
        end
    end

    if normalised && ~realigned
        % Use spm_get to grab "wruns"
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, 'bold', char(func_runs(run))),'wf*.img');
        end
    end

    if realigned && normalised
        % Use spm_get to grab "wrruns"
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, 'bold', char(func_runs(run))), 'wrf*.img');
        end
    end
catch
    fprintf ('=====================================\n');
...
if strcmp(task_flag,'functionals')
    ...
    func_file = sprintf('%s/%s/%s/bold/%s/rf0-0%s-00001-000001-01.img', EXPERIMENT_ROOT_DIR, study, subjID, first_run,first_run);
