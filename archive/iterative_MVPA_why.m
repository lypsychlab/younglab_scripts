...
%                             end
    ...
    if mod(mm, 2) == 1  %if subject number is odd 
        run_array = [3 4 5 8 9 13 14]; % run numbers for WHY trials for odd subjs
    else
        run_array = [1 2 6 7 10 11 12]; % run numbers for WHY trials for even subjs
        
    end

    a = nchoosek(run_array,6);
    for b=1:size(a,1)
        len_b = length(nchoosek(a(b,:),3));
        tester_set(len_b*(b-1)+1:len_b*b,1:3) = nchoosek(a(b,:),3);
    end 
    tester_set = unique(tester_set, 'rows'); %gets rid of repeats

    %returns a matrix containing all possible combinations of the 
    ...
    for nn = 1:numel(masked_vols), %going through each mask
        ...
        if ~isempty(masked_vols{nn}) %skip empty masks.
            ...
            for oo = 1:(numel(tester_set(:,1))); %going through each tester set. 
                training_set = setdiff(run_array,tester_set(oo,:)); %setdiff(A,B) returns runs not in the tester set, makes those the training set
