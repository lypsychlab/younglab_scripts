function younglab_model_spm8_Domint_2mm_smoothed_noSTC_cr(varargin)
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/DomintControlreg'));
            behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
            ...
            results_string = [tasks{subj}{task} '_results_' 'smoothed_' src_data_flag];
            ...
            cd(results_string);
            rS1 = pwd;
			percRetained = freqAnalyzeDomintControlreg(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
            set(0,'DefaultFigureVisible','Off');
...
%                     end
                ...
                spm_defaults = spm_defaults_MVPA;
            ...
            if exist('con_info')
                for k=1:num_runs
                    load(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/DomintControlreg',parafiles{k}),'con_info')
