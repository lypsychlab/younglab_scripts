...
% 10: neutral int
	...
	root_dir='/home/younglw/server/englewood/mnt/englewood/data/PSYCH-PHYS';
...
% 	subj_nums=[3:20 22:24 27:35 38:42 44:47];
	...
	for meow=1:length(kitty)
		...
		all_key=[];all_design=[];
		for rn=1:6
			f=load([kitty{meow} '.DIS.' num2str(rn) '.mat']);
			if ismember(0,f.key)
				missing_flag=1;
			end
		...
		if missing_flag == 1
			...
			for it = 1:length(items)
				...
				for it_2 = 1:length(items)
					...
					simil = (1-(abs(all_key(it)-all_key(it_2))/4))*max([all_key(it) all_key(it_2)]);
