function percRetained = freqAnalyze_itemwise_DIS(study, subj, task, filter, useDir)
...
coF		= filter; 
curD	= pwd;
expDir	= '/home/younglw/server/englewood/DIS_MVPA/';
withSPM = 1;
SPM.RT = 2;
SPM.xBF.T = 128;
SPM.xBF.T0 = 1;
...
cd([expDir study '/behavioural_pp/']);
tT		= dir([subj '.' task '.*']);
...
for cond=1:length(spm_inputs_itemwise)
    for run=1:length(T)
		...
		cndN{cond} = spm_inputs_itemwise(cond).name;
    end
end
...
% grab data 
for run=1:length(T)
    load(T{run},'spm_inputs_itemwise');
    for cond=1:length(spm_inputs_itemwise)    

        for inst=1:length(spm_inputs_itemwise(cond).ons)
            cnd{cond}(run,spm_inputs_itemwise(cond).ons(inst):spm_inputs_itemwise(cond).ons(inst)+spm_inputs_itemwise(cond).dur(inst)) = 1;
        end

		SPM.Sess(run).U(cond).ons    = spm_inputs_itemwise(cond).ons - 1;  % onsets in scans. 
		SPM.Sess(run).U(cond).name   = {spm_inputs_itemwise(cond).name};   % string from spm_inputs.name
		SPM.Sess(run).U(cond).dur    = spm_inputs_itemwise(cond).dur;      % duration in scans from spm_inputs.dur
