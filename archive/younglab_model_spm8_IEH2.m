function younglab_model_spm8_IEH2(varargin)

% younglab_model_spm8
% 
%**************************************************************************
%Last edited 1/13/14 by Amelia, for the purpose of trying out the
%'RT'=>'detail' regressor switch. (search 'detail' to find the locations
%where changes were made, and use comments to toggle between using it and
%RT as the regressor variable.
%***************************************************************************
%
...
%                         regressors
%             *** 1/13/14 ***
%     => 'detail'         This is a test parameter which, for now, just
%                         replaces every instance of the variable 'RT'. It
%                         is a variable consisting of the detail ratings
%                         made by participants during IEH, and one of the things
%                         that we eventually wish to use as a regressor. 
%             ****************
%                         
...
[ErrorLog,SPM]=model(study,subject,tasks,boldirs,src_data_flag,TR,detail_flag,implicit_flag,filter_frequency,maskthresh);
...
function [ErrorLog,SPM]=model(study,subject,tasks,boldirs,src_data_flag,TR,detail_flag,implicit_flag,filter_frequency,maskthresh)
global EXPERIMENT_ROOT_DIR;
global skip_art;
global defaults;
global inStruct;
...
% 					results_string = [results_string '_RT'];
                    results_string = [results_string '_detail'];
				end
				if implicit_flag
...
%             fprintf(fid,'RTs included:          %s\n',rt_text);
            fprintf(fid,'Details included:          %s\n',detail_text);
...
%                     load(parafiles{run},'spm_inputs','con_info','ips','user_regressors','RT','design','item_vector');
                    load(parafiles{run},'spm_inputs','con_info','ips','user_regressors','detail','design','item_vector');
                catch
%                     load(parafiles{run},'spm_inputs','con_info','ips','user_regressors','RT','design');
                    load(parafiles{run},'spm_inputs','con_info','ips','user_regressors','detail','design');
                end
...
%                     fprintf('No ''user_regressors'' variable deteceted for subject %s task `%s`\n',subject{subj}, tasks{subj}{task});
                ...
                % 1/14/14 (now automatically include detail information as a
                % regressor)
                
%                 if RT_flag==1
                if detail_flag==1
%                     fprintf('\nAdding RT information as a regressor for: %s. . . \n',subject{subj});
                    fprintf('\nAdding detail information as a regressor for: %s. . . \n',subject{subj});
%                     try
...
%                             RT = RT(find(d));
%                         end 
                        % if fixation 0's in detail and design
                        if ~isempty(find(d==0)) && length(detail) == length(d)
                            detail = detail(find(d));
                        end 
...
%                         RT(find(RT)) = RT(find(RT))-mean(RT(find(RT)));
                        % mean-center non-missed details
                        detail(find(detail)) = detail(find(detail))-mean(detail(find(detail)));

%                         % now make RT regressors
%                         RT_run = zeros(1,ips);
                        % now make detail regressors
                        detail_run = zeros(1,ips);
...
%                         end
                        % detail_run is now a vector of zeros, length=ips, with
                        % each trial onset value set to the mean-centered
                        % detail value. missed trials = 0
                        for i=1:length(tp)
                            detail_run(tp(i):tp(i)+tdur(i)-1) = ones(tdur(i),1)*detail(i);
                        end
                        
%                         % convolve RT with an HRF response
%                         RT_run = hrf_convolve(RT_run);
                        
                        % convolve detail with an HRF response
                        detail_run = hrf_convolve(detail_run);
...
%                         end
                        ...
                        if exist('user_regressors')
                            user_regressors(end+1).name = 'detail';
                            user_regressors(end+1).ons  = detail_run;
                        else
                            user_regressors(1).name = 'detail';
                            user_regressors(1).ons  = detail_run;
                        end
%                     catch
...
%                         n=numel(SPM.Sess(k).col)-(numel(con_info(contrast).vals)+(exist('RT') & RT_flag==1));
                        n=numel(SPM.Sess(k).col)-(numel(con_info(contrast).vals)+(exist('detail') & detail_flag==1));
                        if n>0,
