function filename = conn_BATCH_setup_MOR4(varargin)
% ================================================
...
%=================================================
	...
	cd(d);
        ...
        for sess=1:length(sessions{sub}) %for every session specified for this subject
            ...
            funcdir_tmp=dir('swr*.img'); %find all the preprocessed images for that session (there will be many)
            funcdir=[];
...
%     end
	...
	if default_roi
		...
		BATCH.Setup.rois=make_roi_struct_MOR4(study,subjIDs); %
	end
