...
input_runs = {6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6,6,6,6,6,5,6,5};
num_conds = 10;

study_path = '/younglab/studies/DIS_MVPA/';
model_path = '/results/DIS_results_smoothed_normed';
outputfolder = '/younglab/studies/DIS_MVPA/MVPA_jordan';
outputname = 'collecting_betas_jorieModel';

condition_names = {'A_PHA','B_PSA','C_IA','D_PA','E_NA','F_PHI','G_PSI','H_II','I_PI','J_NI'};%{'A_HARM','B_PURITY'};These should match the order that they are listed in spm_input in the behavioural files.
...
contrast_names = {'IntPhysHarm_v_AccPhysHarm','IntPsyHarm_v_AccPsyHarm'};%{'Purity_v_Harm'};
contrast_nums = {[1 6;2 7]}; %{[1 2]};
...
%              maskFiles{xx} = spm_read_vols(spm_vol(roi_file.name));
        ...
        for i = 1:numel(maskFiles),
            ...
            %Amelia - adding this bit to the end of the loop to grab masked
            %betas and throw them into the combining_betas folder of
            %DIS_MVPA
            %cd /younglab/studies/DIS_MVPA/combining_betas
            %csvwrite([experiments.data{1,mm}, '.DIS_results_smoothed_normed_maskedvols.xlsx'],masked_vols)
        end
