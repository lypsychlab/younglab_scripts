function searchlight_all_regress_training_finished(rootdir,study,subj_tag,resdir,sub_nums,conditions,sph,B_in,outtag)
...
% These must be located in your studys behavioural directory.
...
% - in corrs*.mat and spear*.mat, the LAST value represents the constant (intercept) regressor.
	...
	addpath(genpath('/usr/public/spm/spm12')) % add the path to SPM12 on pleiades, plus all its subdirectories

	for sub=1:length(sub_nums)
		subjIDs{1,sub}=[subj_tag '_' num2str(sprintf('%02d',(sub_nums(sub))))]

    % set up an array of coordinates for a sphere of size sph centered at (0,0,0)
    % these will be used later to grab all the coordinates of the voxels in the sphere around a chosen voxel
	spheresize=(2*sph)-1;
	...
	% CD to the directory /home/younglw/scripts
	cd(fullfile(['/home/' 'younglw/' 'scripts/']))

	% load arrays of ordered voxels and a grey matter mask:
	load voxel_order2; 
	...
	for subj=1:length(subjIDs) %grabbing beta images
		...
		% start the clock
		B=[];
		% initialize an empty array called B
		
		% load up the premade matrix regressors:
		for b=1:length(B_in)
			try
				...
				B=[B behav_matrix];
				clear behav_matrix 
				% remove the variable behav_matrix from the workspace
			catch
		...
		% change directories so youre in the subdirectory named by the parameter "resdir"

		% grab your starting index and the number of images you want to get:
	    c1=conditions(1);
	    conditions=length(conditions);

		betadir=dir(['*beta_item*.nii']) % set the variable betadir to a listing of every file that starts with "beta_item" and ends with ".nii"
		
		betafiles=cell(conditions,1) % set the variable betafiles to an empty cell array with the dimensions (conditions) x 1

		% put the names of your beta images into betafiles, and read the volumes with spm:
		for i=1:conditions
	    ...
	    % set up some empty arrays for later:
	    ...
	    spear=[];
	    % iterate through every voxel in the brain, drawing a sphere around it
	     % and loading the beta values of voxels in that sphere:
	    for i = 1:length(greymattermask2)% for each voxel
	        ...
	        spherebetas_1stcol=spherebetas(:,1);
	        goodrows=length(find(isnan(spherebetas_1stcol)<1));

			% make a variable called goodrows that equals the number of nonzero numbers in the first column of spherebetas
			% keep in mind that this column might also contain NaNs
			% ex. if the first column were [.23; .46; .82; NaN; .65; 0], goodrows = 4
			...
			if length(goodrows) > 9 % if there are at least 10 good voxels in this sphere
	            ...
	            % run the regression of beta values on regressors:
	            predictors = horzcat(ones(size(B,1),1),B);
	            weights = regress(bigmat(i,:)',predictors);

	            weights = weights.'
	            % transpose the vector called "weights"

	            corrs(i,:)=[weights(2:end) weights(1)];

	            % run a spearman rank correlation:
	            ...
	            % add thisspear (a row vector) to the bottom of spear, and set that equal to spear


	        end
	    end
	    clear temp simmat B weights predictors thisspear

	    save(['output_vars' outtag '.mat'],'bigmat','corrs','spear')
	    % save the variables bigmat, corrs, and spear in an output file named "output_vars" + outtag + ".mat"
	    % ex. if outtag is "_test", "output_vars_test.mat"


	    disp('Correlations saved.');

	    % initialize some empty output brain maps:
        ...
        % fill in each map with the values for the corresponding regressor:
        for b=1:(length(B_in)+1)

		     
		    for i=1:length(greymattermask2)
		    % use a for statement to loop through every ith voxel in greymattermask2
		    % the end statement is already complete for you
	    ...
	    % get some volume information you can use to write out brain images:
	    ...
	    for b=1:length(B_in)
		    ...
		    cd(fullfile(['/home/' 'younglw/' 'scripts/']));
		    map=corrmap(b).map;
		    spm_write_vol(template,map);
		    % call the function spm_write_vol() with two arguments:
		    % template (the volume information you obtained with spm_vol),
		    % and map. 
		    % map should be set to the map field of the bth element of the structure corrmap. 
		end
		template.fname = ['RSA_searchlight_regress_const_' outtag '.img']; 
		
		map=corrmap((length(B_in)+1)).map;
		spm_write_vol(template,map);
		% call the function spm_write_vol() with two arguments:
	    % template (the volume information you obtained with spm_vol),
	    % and map. 
	    % map should be set to the map field of the (n+1)th element of the structure corrmap,
	    % where n = the length of B_in
