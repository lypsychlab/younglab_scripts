function DIS_Combcond_withControl(subjID)
...
%     xx(3).name = 'C_NEUTRAL';
%     xx(3).ons = sort([spm_inputs(5).ons, spm_inputs(10).ons]);
%     xx(3).dur = sort([spm_inputs(5).dur; spm_inputs(10).dur]);
%     
%    spm_inputs = xx;
%    clear xx
% %         
...
%     
% cd /younglab/studies/DIS_MVPA/behavioural/controlConditionRegressor
%     save([subjID '.DIS.domainWcontrol.' num2str(acq) '.mat' ],'acq','RT','key','design','design_run','items','items_run','ips', 'con_info', 'spm_inputs', 'experimentDur', 'subjID'); %'user_regressors');
%     clear acq RT key design design_run item items_run ips con_info spm_inputs experimentDur xx; %user_regressors;
% 
% end
...
for file=1:length(files)
    ...
    yy(5).name = 'C_NEUTRAL';
    yy(5).ons = sort([spm_inputs(5).ons, spm_inputs(10).ons]);
    yy(5).dur = sort([spm_inputs(5).dur; spm_inputs(10).dur]);
...
cd /younglab/studies/DIS_MVPA/behavioural/controlConditionRegressor
    save([subjID '.DIS.domintWcontrol.' num2str(acq) '.mat' ],'acq','RT','key','design','design_run','items','items_run','ips', 'con_info', 'spm_inputs', 'experimentDur', 'subjID'); %'user_regressors');
    clear acq RT key design design_run item items_run ips con_info spm_inputs experimentDur xx; %user_regressors;
