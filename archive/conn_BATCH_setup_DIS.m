...
%=================================================
	...
	param_names={'filt' 'confounds' 'steps' 'filename' 'conditions'};
	...
	% 	BATCH.New.structurals{sub}='%path to str'roi'},'choose_roiuct image'
	...
	cd(d);
        ...
        for sess=1:length(sessions{sub}) %for every session specified for this subject
            if sessions{sub}(sess)<10
               sessdir=fullfile(EXPERIMENT_ROOT_DIR,study,subjIDs{sub},'bold',['00' num2str(sessions{sub}(sess)) '/']);
            else
            ...
            funcdir_tmp=dir('swrf*.img'); %find all the preprocessed images for that session (there will be many)
            funcdir={};
