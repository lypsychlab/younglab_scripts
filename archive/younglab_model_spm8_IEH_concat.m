function younglab_model_spm8(varargin)

% Last edited by Brendan Gaesser 01/01/2015.

% This script has been modified to run SPM for concatenated runs 
% (i.e., all runs combined serially into a single run). If you want to
% analyze runs as individual runs (conventional analysis), use
% younglab_model_spm8 script.


% BEFORE RUNNING SCRIPT THE FOLLOWING PARAMETERS NEED TO BE TAILORED TO THE
% SPECIFIC STUDY DESIGN:
%**************************************************************************
%**TR (a.k.a, repetition time) is the time it takes to complete one scan of
%the entire brain. Experimental tirals are typically designed in units of
%TR
% 1) Set TR IN SECONDS 
TR = 2.5;

% 2) SET # OF SCANS PER BOLD RUN. this code that uses this variable will
% need to be adapted if the number of scans varies across runs. THIS MUST ALSO BE INPUT AFTER DESIGN CONFIGURATION
% AND ESTIMATION (near lines 884,927)
runlength = 167

% 3) SET # of BOLD RUNS. THIS MUST ALSO BE INPUT AFTER DESIGN CONFIGURATION
% AND ESTIMATION (near lines 884,927)
numruns = 8

% 4) SET ONESETS AS EITHER IN UNITS OF SECONDS OR TRs
% change line "SPM.xBF.UNITS      = 'secs'" below
%**************************************************************************
...
%       ==> would attempt to model  audio fb task (fba) for bold runs 004,
%           008, and 014 in a new directory called
...
%***********# of runs set to 1 in order to concatenate all runs into a single run**********
            num_runs = 1; 
            % previously "num_runs = length(boldirs{subj}{task});"
            if num_runs == 1
                fprintf('Data will be analyzed using a single concatenated run');
            end
...
%*************** Get scans for each bold run and then combine them into a
                % single scans variable. The # of rows for the scans variable (i.e., the
                % total number of scans across all runs) should equal ips
                % when analyzing data for concatenated runs.***************
                fprintf('loading image files for all runs \n');
                for runnum = 1:length(boldirs{subj}{task}) 
                    scans = [scans; alek_get(sprintf('%s/%s/%s/bold/%.3d',EXPERIMENT_ROOT_DIR,study,subject{subj},boldirs{subj}{task}(runnum)),filter)];
                end
   
                % scanadd{runnum} = alek_get(sprintf('%s/%s/%s/bold/%.3d',EXPERIMENT_ROOT_DIR,study,subject{subj},boldirs{subj}{task}(runnum)),filter);
...
%***********This bit of code attempts to reformat the run mean regressors as block effects (.iB)
           %instead of covariate effects (.iC), since covariates are
           %convolved with hrf and run mean regressors should not be
           %convlved with hrf. Despite this code, spm_spm (i.e. estimate
           %parameters) still convolves run mean regressors with hrf. I
           %have written new code to directly change the design matrix
           %post-hoc after spm_spm, but if there is time one day I aspire
           %to pick up the trail of .iB and .iC and solve the problem
           %pre-spm_spm as I think this would be a more elegant solution.
            
            % # of BOLD RUNS. 
            numruns = 8
           
            % # of runs - 1. For example, if # of runs = 8. numrunsmodeled = 7.
            numrunsmodeled = (numruns - 1)

            % 2) SET # OF SCANS PER BOLD RUN. this code that uses this variable will
            % need to be adapted if the number of scans varies across runs. 
            runlength = 167
            
             % # of trials across all runs
            allruns = length(SPM.xX.X)

            SPM.xX.iC(length(SPM.Sess.U)+1:length(SPM.Sess.U)+numrunsmodeled) = []

            for runindex = 1:numrunsmodeled
                SPM.xX.iB(runindex) = length(SPM.Sess.U)+runindex 
                SPM.xX.X(:,runindex+length(SPM.Sess.U)) = zeros(1, allruns)
                SPM.xX.X((runlength*runindex)-(runlength-1):runlength*runindex,(runindex+length(SPM.Sess.U))) = ones(1,runlength)
            end

            SPM.xX.iB(runindex+1) = length(SPM.Sess.U)+runindex+1 
            save SPM SPM
%***********    
...
%***********This bit of code directly changes the design matrix
           %post-hoc after spm_spm, but if there is time one day I aspire
           %to pick up the trail of .iB and .iC and solve the problem
           %pre-spm_spm (see above)
           ...
           % # of BOLD RUNS. 
            ...
            for runindex = 1:numrunsmodeled
                %design matrix of trials and betas
                SPM.xX.xKXs.X(:,runindex+length(SPM.Sess.U)) = zeros(1, allruns)
                SPM.xX.xKXs.X((runlength*runindex)-(runlength-1):runlength*runindex,(runindex+length(SPM.Sess.U))) = ones(1,runlength)
                %design matrix (SPM.xX.xKS.X) scaled for display
                SPM.xX.nKX(:,runindex+length(SPM.Sess.U)) = zeros(1, allruns)
                SPM.xX.nKX((runlength*runindex)-(runlength-1):runlength*runindex,(runindex+length(SPM.Sess.U))) = ones(1,runlength)
            end
...
%***********    
            ...
            for contrast=1:length(con_info)
                % if task is only 1 run, uncomment the following line for regressors
               % con_vals{contrast}=[con_vals{contrast}, zeros(1,num_runs + num_regressors)]; %zero pad for effects of interest
               % THIS BIT OF CODE SHOULD PROBABLY BE REMOVED AS IT OVERPADS
               % THE EFFECTS OF INTEREST WITH ZEROS.
...
%******PROBLEM: Artifcat script "art('sess_file', cfgfile);" is erroring out using concatenated runs
%******It looks like the problem occurs because the movement files for only the first bold run (i.e., session) 
%******are being loaded (see M = {} in art.m script). This is occuring because the loading of movement files 
%******for a different bold run is triggered each time the script reads the word "session" in the art_config001.cfg in a subjects results 
%******folder. Since "session" is only appearing once, only the movement files for the first bold run are being loaded
%******SOLUTION: 1) Might involve tweaking the M = {} art.m script to concatenate movement files from additional bold runs. 
%******          in the art.m script  
%******          OR
%******          2) Insert "session" in between bold runs in the art_config
%******          below be reading SPM.XY.P to figure out where the breaks
%******          in the bold runs are. Or re-concatenating the bold run
%******          trials from scratch here.
%                

%         Might be useful for thinking about solution: SPM.Sess(1).row(1,1:167)  
%         
%         
