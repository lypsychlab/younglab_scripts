function percRetained = freqAnalyzeDomainControlreg(study, subj, task, filter, useDir)
...
cd([expDir study '/behavioural/DomainControlreg']);
tT		= dir([subj '.' task '.*']);
...
for run=1:length(T)
    ...
    for cond=1:length(spm_inputs)    
		...
		SPM.Sess(run).U(cond).dur    = spm_inputs(cond).dur;      % duration in secs from spm_inputs.dur
