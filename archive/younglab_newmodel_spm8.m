function younglab_newmodel_spm8(varargin)

% younglab_newmodel_spm8
% 
...
%
% E.G. younglab_newmodel_spm8('BLI','YOU_BLI_01','fba',[4 8 14])
...
%                         regressors
%
%     => 'no_stc'         The script will expect non-slice-time-corrected data
%                         
...
% Modified by LT - 10/12/2012 Notes: added in slice-time correction
...
if nargin == 0
    help younglab_newmodel_spm8;
    return;
elseif nargin<4
    disp('error: younglab_newmodel_spm8 requires a minimum of 4 arguments');
...
if ~iscell(z)
    ...
    z =regexp(z,'/','split');
    EXPERIMENT_ROOT_DIR = fullfile('/',z{1}{2},z{1}{3});
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% process tasks
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tic;
[ErrorLog,SPM]=model(study,subject,tasks,boldirs,stc_flag,src_data_flag,TR,RT_flag,implicit_flag,filter_frequency,maskthresh);
...
%-------------------------------------------------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ErrorLog,SPM]=model(study,subject,tasks,boldirs,stc_flag,src_data_flag,TR,RT_flag,implicit_flag,filter_frequency,maskthresh)
global EXPERIMENT_ROOT_DIR;
global skip_art;
global defaults;
global inStruct;
...
if strmatch(src_data_flag,'normed') & strmatch(stc_flag,'stc')
    filter = 'swraf*img';
elseif strmatch(src_data_flag,'unnormed') & strmatch(stc_flag,'stc')
    filter = 'sraf*img';
elseif strmatch(src_data_flag,'normed') & strmatch(stc_flag,'no_stc')
    filter = 'swrf*img';
elseif strmatch(src_data_flag,'unnormed') & strmatch(stc_flag,'no_stc')
    filter = 'srf*img';
else
        ...
        for task = 1:length(tasks{subj})
            ...
            if num_behav_files
                ...
                for i=1:num_behav_files
                    parafiles{i}=behavdata_thistask(find(vec==sorted_vec(i))).name; % finally reorganize w/ find
                end
            else
            ...
            % all variables with ad prefix are used for artifact file detection.
            ad_bdirs = boldirs{subj}{task}; 
            ad_aff = 1; % artifact detection all files found boolean
            ad_files = {};
            ad_suffixes = {'_with_art_and_mot_reg','_with_art_reg','_with_mot_reg'};
            prompts = {'It looks like you have run an artifact analysis. Would you like to model them?', ...
                       'Yes - Artifacts and Motion','Yes - Artifacts only','Yes - Motion only','No'};
            for ad_i = ad_bdirs
                ad_file = adir(fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj},'bold',sprintf('%03i',ad_i),'art_regression_outliers_and_movement*.mat'));
                if ~iscell(ad_file)
                    ad_aff = 0;
                    break
                end
                ad_files{end+1} = ad_file{1};
            end
            if ad_aff
                ad_choice = menu(prompts{:});
                if ad_choice == 4
                    ad_choice = 0;
                    ad_suffix = '';
                else
                    skip_art = 1;
                    ad_suffix = ad_suffixes{ad_choice};
                end
            else
                ad_choice = 0;
                ad_suffix = '';
            ...
            if ad_choice
                for ad_i = 1:length(ad_bdirs)
                    ad_dat = load(ad_files{ad_i});
                    ad_mrn = {'x_trans','y_trans','z_trans','roll','pitch','yaw'};
                    ad_ur(ad_i).ur = [];
                    if ad_choice==1||ad_choice==2
                        % Insert artifacts
                        for ad_j = 1:size(ad_dat.R,2)-6
                            ad_ur(ad_i).ur(end+1).name = sprintf('artifact_regressor_run_%i_num_%i',ad_i,ad_j);
                            ad_ur(ad_i).ur(end).ons  = ad_dat.R(:,ad_j);
                        end
                    end
                    if ad_choice==1||ad_choice==3
                        % Insert motion
                        for ad_j = 1:6
                            ad_ur(ad_i).ur(end+1).name = sprintf('%s_run_%i',ad_mrn{ad_j},ad_i);
                            ad_ur(ad_i).ur(end).ons = ad_dat.R(:,end-6+ad_j);
                        end
                    end
                end
            end
            %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            scans = [];
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj}));
            mkdir(fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj},'report'));
            comd = rlc();
            save(fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj},'report','model_command'),'comd');
            f=fopen(fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj},'report','model_command'),'a');
            fprintf(f,'%s\n',comd);
            fclose all;
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj},'results'));
            % BUILD RESULTS STRING 
            % (constructs an appropriate name for the results)
            results_string = [tasks{subj}{task} ad_suffix '_results_' src_data_flag];
...
%                     fprintf('No ''user_regressors'' variable deteceted for subject %s task `%s`\n',subject{subj}, tasks{subj}{task});
				...
				if max(vertcat(spm_inputs(:).ons)) > ips
...
%                     end
                ...
                if ad_choice
                    % add in artifact / motion regressors
                    for reg = 1:length(ad_ur(run).ur)
                        SPM.Sess(run).C.C(:,end+1) = ad_ur(run).ur(reg).ons;
                        SPM.Sess(run).C.name = [{SPM.Sess(run).C.name{:}} {ad_ur(run).ur(reg).name}];
                    end
                end
            ...
            %===========================================================================
                ...
                if implicit_flag == 1
                    ...
                    younglab_genmask(study,subject{subj},0);
                    SPM.xM.T           = SPM.xM.T;
                    SPM.xM.TH          = SPM.xM.TH * -Inf;
                    SPM.xM.I           = 0;
                    SPM.xM.VM          = spm_vol([fullfile(EXPERIMENT_ROOT_DIR,study,subject{subj},'3danat') '/skull_strip_mask.img']);
                    %SPM.xM.xs.Masking  = sprintf('Skull-stripped mask, made with FSL, threshold: %0.01f',maskthresh);
                    SPM.xM.xs.Masking  = 'Mask produced using younglab_genmask';
                end
            ...
            for k=1:num_runs
                ...
                if exist('con_info')
                    for contrast=1:length(con_info)
                        ...
                        elseif ad_choice
                            con_vals{contrast} = [con_vals{contrast} con_info(contrast).vals zeros(1,length(ad_ur(k).ur))];
                        else
            ...
            if exist('con_vals','var')
                if size(SPM.xX.X,2)~=size(con_vals{1},2)
                    warning('Number of contrast values does not match number of regressors! Contrasts will not be written correctly');
                end
            end
            for contrast=1:length(con_info)
...
if inStruct.saveim
    saveas(get(0,'children'),sprintf('artifact_analysis_%s_%s.png',artsubj,arttask));
end
close(get(0,'children'));
fprintf('\nCleaning up...\n');
system('rm -rf art_mask_temporalfile.mat');
end
