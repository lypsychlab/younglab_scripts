function younglab_model_spm8_TOM(varargin)
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/behavioural_TOM'));
            behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
            ...
            cd(results_string);
            rS1 = pwd;
			percRetained = freqAnalyze_TOM(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
            set(0,'DefaultFigureVisible','Off');
...
%                     end
            ...
            %---------------------------------------------------------------------------
            for k=1:num_runs
                load(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/behavioural_TOM',parafiles{k}),'con_info')
                if exist('con_info')
