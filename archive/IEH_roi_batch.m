function IEH_roi_batch(subjects,roi_name,task_dir, loc_dir, contrast_num, win_secs, onsetdelay, highpass, meanwin)
%
% 1/31/14 Amelia 
%
%  NOTES ON INPUT ARGUMENTS:
%  All of the inputs are essentially just the selections that you would
%  normally make when running the batch GUI
%
%  subjects     = an array of subjects with the syntax
%  roi_name     = the roi name as listed (spelled/capitalized) in the roi library
%  task_dir     = the directory of the task SPM file (e.g
%  loc_dir      = the directory of the task SPM file which was used to
%                 localize the roi
%  contrast_num = the number contrast that you wish to run as listed in the task
%                 SPM
%  win_secs     = the time window in seconds (default:
%  onsetdelay   = the onset delay in seconds (default:
%  highpass     = if yes: 1 ; if no: 0 (default:
%  meanwin      = if 
% 
% 
% 
