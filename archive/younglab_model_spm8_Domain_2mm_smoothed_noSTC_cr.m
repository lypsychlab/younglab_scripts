function younglab_model_spm8_Domain_2mm_smoothed_noSTC_cr(varargin)
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/Domain'));
            behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
            ...
            cd(results_string);
            rS1 = pwd;
			percRetained = freqAnalyzeDomain(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
            set(0,'DefaultFigureVisible','Off');
...
%                     end
            ...
            if exist('con_info')
                for k=1:num_runs
                    load(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/Domain',parafiles{k}),'con_info')
