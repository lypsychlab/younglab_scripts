...
temp       = textscan(task_dir,'%s','delimiter','/'); temp = temp{:};
task       = temp{end};

temp       = textscan(loc_dir,'%s','delimiter','/'); temp = temp{:};
loc        = temp{end};

fb_sad_subjs = {subjects{1:13}};
new_fbv_subjs = {subjects{14:30}};
tomloc_subjs = {subjects{31:end}};


temp       = textscan(subjects{1},'%s','delimiter','/'); temp = temp{:};
study_dir  = [temp{1} '/' temp{2} '/' temp{3} '/' temp{4}];

% meanwin    = strread(meanwin,'%s');

contrast_num = strread(contrast_num,'%s','delimiter',';'); contrast_num = contrast_num{:};
...
%      try
    if (exist((fullfile(subjects{s},task_dir,'SPM.mat')))== 0) 
        ...
        if group_roi==0;
            ...
            if strmatch(subjects{s},fb_sad_subjs)
                loc = 'fb_sad_results_normed_smoothed';
            elseif strmatch(subjects{s},new_fbv_subjs)
                loc = 'new_fbv_results_normed_smoothed';
            else
                loc = 'tomloc_results_normed_smoothed';
            end
            
            
            temp = dir(fullfile(subjects{s},'roi',['ROI_' roi_name '_' loc '_' contrast_num '_' '*xyz.mat']));
            if isempty(temp)
                temp = dir(fullfile(subjects{s},'roi',['ROI_' roi_name '_' 'tomloc_results_normed_smoothed' '_' contrast_num '_' '*xyz.mat']));
            end
...
%         end
        ...
        for t = 1:length(V)
            ...
            new_filepath = strsplit(V(t).fname, '/');
            V(t).fname = fullfile('/home','younglw',new_filepath{4:end});
        ...
        if exist('notes','var')== 0
            ...
            notes        = cell(length(Cond)*length(Cond(1).onidx)*length(subjects)+2,window_length+4); %6);
            if group_roi==1
            ...
            for i=1:window_length % -1:window_length
                notes(2,i+4) = {(i*RT)-RT}; % notes(2,i+6) = {(i*RT)-RT}; %This sets up all the TR onsets.
            end
...
%             end
        ...
        for c = 1:length(Cond) %for each condition...
            ...
            for t=1:window_length %+2 % include -1 and 0 trs %for each TR
                tmp_idx          = Cond(c).onidx + (t-1); %+ (t-3); %So this finds the onset and eeks along after it by TR.
