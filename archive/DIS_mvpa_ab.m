...
experiments=struct(...
    'name','DIS_2mm',... % study name
    'pwd1','/younglab/studies/DIS_2mm/',...   % folder with participants 
    'pwd2','results/DIS.domain_results_smoothed_normed/',...   % inside each participant, path to SPM.mat
...
partition_names={'Even_','Odd_'};

 condition_names_all={{'purity','harm'}%,... XXXcheck shapes and stop here
...
for i=1:length(roiFiles)
    roin = roiFiles{i};
    roinum = i;
    savedirectory = '/younglab/studies/DIS_2mm/MVPA_replicate/MVPA_domain_smoothed_normed_nondirectional';
    mkdir(savedirectory);
