function younglab_preproc_spatial (varargin)
%
...
if nargin==1
    study    = varargin{1};
    subj_dir_identifier = 'MVPA_searchlight';
...
try
    if ~realigned && ~normalised
        ...
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, char(func_runs(run))),'Y*.nii');
        end
    end
    ...
    if realigned && ~normalised
        ...
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, char(func_runs(run))), 'rY*.nii');
        end
    end
    ...
    if normalised && ~realigned
        ...
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, char(func_runs(run))),'wY*.nii');
        end
    end
    ...
    if realigned && normalised
        ...
        for run = 1:length(func_runs)
            func_images{run} = alek_get(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, char(func_runs(run))), 'wrY*.nii');
        end
    end
catch
    fprintf ('=====================================\n');
