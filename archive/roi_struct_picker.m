function roi_struct_picker(t,k,r,c,roi_name,roi_xyz,subjects,res_dir)
...
if i>length(subjects) % just finished last subject
    ...
    if ischar(roi_xyz)
        try   eval(['spm_mip_ui(''SetCoords'',[' roi_xyz' '],findobj(spm_figure(''FindWin'',''Graphics''),''Tag'',''hMIPax''));']); %So this sets the cursors to
        catch eval(['spm_mip_ui(''SetCoords'',' roi_xyz' ',findobj(spm_figure(''FindWin'',''Graphics''),''Tag'',''hMIPax''));']); %The ROI specified, then the next item
        end
        %spm_mip_ui('Jump',findobj(spm_figure('FindWin','Graphics'),'Tag','hMIPax'),'nrmax'); % This one.. makes the cursors jump to the local maximum.
    else
        % Set the cursor to the starting location for this ROI. If ROI
        % wasn't set manually then it will pull an array that defines all
        % the possible voxels the ROI could define. That is what is in the
        % functional library.
        h  = findobj(spm_figure('FindWin','Graphics'),'Tag','hMIPax'); % Get Handle for the SPM figure
        MD = get(h,'UserData');% MD.Z is a single vector of z scores for all suprathreshold voxels *****So this looks like a place to start for structural. All above
                                % threshold voxels are here. We'd need to
                                % set a lower threshold first. This user
                                % data should be defined through the spm
                                % function that brings up the glass brain
                                % in the first place.
        [null, null, roi] = intersect(roi_xyz,MD.XYZ','rows'); % MD.XYZ is a 3-row matric of coordinates of MD.Z voxels. So for a user specified ROI there is just one 
                % value for roi_xyz. If we used a library file then there
                % should be a bunch. The next bit brings you to the highest
                % voxel within the possible ROI region.
        if isempty(roi)
            warndlg(['No Suprathreshold voxels for ' roi_name '!']);
        else
            [null, idx] = max(MD.Z(roi)); %Find highest z score within those that can be within ROI
            xyz         = MD.XYZ(:,roi(idx)); %Grab the corresponding 3 rows to that z score. 
...
%try   evalin('base','xY.xyz = spm_mip_ui(''SetCoords'',[' roi_xyz' '],findobj(spm_figure(''FindWin'',''Graphics''),''Tag'',''hMIPax''));' %So this sets the cursors to
%        catch evalin('base', 'xY.xyz = spm_mip_ui(''SetCoords'',' roi_xyz'
%        ',findobj(spm_figure(''FindWin'',''Graphics''),''Tag'',''hMIPax''));' %The ROI specified, then the next item
%end

%evalin('base','xY.xyz     = spm_mip_ui(''Jump'',findobj(spm_figure(''FindWin'',''Graphics''),''Tag'',''hMIPax''),''nrmax'');'); %This jumps to local maximum, change this.
