...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural','design_behave'));
            behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
            ...
            cd(results_string);
            rS1 = pwd;
			percRetained = freqAnalyze_XPECT_D(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
            set(0,'DefaultFigureVisible','Off');
...
%                     end
            ...
            if exist('con_info')
                for k=1:num_runs
                    load(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural','design_behave', parafiles{k}),'con_info')
