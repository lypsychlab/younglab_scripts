function younglab_model_spm12_itemwise__timesplit_unsmooth_pleiades(varargin)
% THIS VERSION modified to a) produce itemwise beta files with 2 files per item (1 for each time portion) and b) always find unsmoothed data.
% This is useful for modeling pre-RSA.
% It also assumes that: you already have
% skull_strip_masks; and that you have itemwise_timesplit inputs in your behavioural .mats
% in the variable spm_inputs_itemwise_timesplit.
%
% younglab_model_spm8
% 
...
% 'spm_inputs_itemwise_timesplit' in an individual mat file for every run that is to be
...
%       spm_inputs_itemwise_timesplit is a scructure with fields name, ons, and dur
%
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            clear spm_inputs_itemwise_timesplit con_info ips scans; SPM=[];
            if strmatch(src_data_flag,'normed')
                filter = 'wraf*img';
            ...
            results_string = [tasks{subj}{task} '_results_itemwise_timesplit_' src_data_flag];
            ...
            for run = 1:num_runs
                ...
                clear RT
                % try
                    load(parafiles{run},'spm_inputs_itemwise_timesplit','con_info','ips','user_regressors','RT','design','item_vector');
                % catch
                %     load(parafiles{run},'spm_inputs_itemwise_timesplit','con_info','ips','user_regressors','RT','design');
                % end
                if ~exist('spm_inputs_itemwise_timesplit') | ~exist('ips')
                    fprintf('Failed to load behavioural data for subject %s task `%s`\n',subject{subj}, tasks{subj}{task});
                    fprintf('\t-variables "spm_inputs_itemwise_timesplit" and "ips" required (and "con_info" is handy)\n');
...
%                     fprintf('No ''user_regressors'' variable deteceted for subject %s task `%s`\n',subject{subj}, tasks{subj}{task});
				...
				if max(horzcat(spm_inputs_itemwise_timesplit(:).ons)) > ips % saxelab uses vertcat but horzcat should do the same thing
					warning('\n----------------------------------------\nYour maximum onset: %.0f is greater than your ips (%.0f)!\n',max(vertcat(spm_inputs_itemwise_timesplit(:).ons)),ips);
					...
					elseif strcmpi(onset_resp,'c')
						...
						for onsTask = 1:length(spm_inputs_itemwise_timesplit)
							spm_inputs_itemwise_timesplit(onsTask).ons = spm_inputs_itemwise_timesplit(onsTask).ons/2+1;
						end
					end
				end
                for cond = 1:length(spm_inputs_itemwise_timesplit)
                    ...
                    % image is 1.
                    SPM.Sess(run).U(cond).ons    = spm_inputs_itemwise_timesplit(cond).ons - 1;  % onsets in scans. 
                    SPM.Sess(run).U(cond).name   = {spm_inputs_itemwise_timesplit(cond).name};   % string from spm_inputs_itemwise_timesplit.name
                    SPM.Sess(run).U(cond).dur    = spm_inputs_itemwise_timesplit(cond).dur;      % duration in scans from spm_inputs_itemwise_timesplit.dur
...
%                     try
                        ...
                        for i=1:length(spm_inputs_itemwise_timesplit)
                            tp   = [tp spm_inputs_itemwise_timesplit(i).ons'];
                            tdur = [tdur spm_inputs_itemwise_timesplit(i).dur'];
                        end
...
%                         disp('    -# non-fixation onsets in RT, design, and spm_inputs_itemwise_timesplit do not match')
...
%                     end
                ...
                clear spm_inputs_itemwise_timesplit ips user_regressors
            end % run/acq loop
