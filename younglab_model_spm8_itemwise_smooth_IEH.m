function younglab_model_spm8_itemwise_smooth_IEH(varargin)
% THIS VERSION modified to produce itemwise beta files.
% It also assumes that: your study is under /younglab/studies; that you have itemwise inputs in your behavioural .mats
% in the variable spm_inputs_itemwise; and that your TR = 2.5 (for
% IEHFMRI).
%
% younglab_model_spm8
% 
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(results_string);
            rS1 = pwd;
			percRetained = freqAnalyze_itemwise_IEH(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
            set(0,'DefaultFigureVisible','Off');
...
% 				if max(horzcat(spm_inputs_itemwise(:).ons)) > ips % saxelab uses vertcat but horzcat should do the same thing
...
% 					elseif strcmpi(onset_resp,'c')
            % TO AUTOCONVERT (2.29.2016)
%                 (max(horzcat(spm_inputs_itemwise(:).ons))/TR)
% 						disp('Fixing onsets for behavioural file %s. Note that this fix is not permanent.',parafiles{run});
						for onsTask = 1:length(spm_inputs_itemwise)
							spm_inputs_itemwise(onsTask).ons = spm_inputs_itemwise(onsTask).ons/TR +1;
                            spm_inputs_itemwise(onsTask).dur = spm_inputs_itemwise(onsTask).dur/TR;
						end
% 					end
% 				end
...
%     end
    if ~exist([directory 'skull_strip_mask.img'],'file') | mask_over
        ...
        structural_dir=dir([directory 'ws*']);
        if length(structural_dir)>1
            fprintf('Too many structural files! Removing .img/keeping .nii.\n');
            cmd1 = sprintf('/bin/bash --login -c ''rm -rf %s''',[directory 'ws*img']);
            cmd2= sprintf('/bin/bash --login -c ''rm -rf %s''',[directory 'ws*hdr']);
            system(cmd1);
            system(cmd2);
        end
