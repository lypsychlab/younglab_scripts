function roi_batch_itemwise_run_baseline_mktrag(subjects,roi_name,task_dir, loc_dir, contrast_num, win_secs, onsetdelay, highpass, meanwin)
...
for s = 1:length(subjects)
    ...
    try
        if (exist((fullfile(subjects{s},task_dir,'SPM.mat')))== 0)
            ...
            run_1_ol = offlist <= 139;
            run_2_ol = offlist >=140 & offlist<=278;
            run_3_ol = offlist >=279 & offlist<=417;
            run_4_ol = offlist >=418 & offlist<=556;
            run_5_ol = offlist >=557 & offlist<=695;
            run_6_ol = offlist >=696 & offlist<=834;
