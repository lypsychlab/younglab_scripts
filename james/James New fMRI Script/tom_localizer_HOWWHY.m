...
% 
% E.G. tom_localizer('YOU_SHAPES_01',1);
% 
...
orig_dir			= pwd;
textdir				= fullfile(rootdir, 'tom_localizer_stimuli');
...
if mod(sscanf(subjID(length(subjID)),'%d'),2) == 1
    if run == 1
        design = designs(1,:);
    else
        design = designs(2,:);
    end
elseif mod(sscanf(subjID(length(subjID)),'%d'),2) == 0
    if run == 1
        design = designs(2,:);
    else
        design = designs(1,:);
...
% design				= designs(run,:); (use this if you're not counterbalancing)
conds				= {'belief','photo'};
...
try
	...
	Screen(w, 'DrawText', task, x0-80, y0-60, [255]);
	Screen(w, 'DrawText', instr_1, x0-190, y0, [255]);
	Screen(w, 'DrawText', instr_2, x0-190, y0+60, [255]);
...
try
	for trial = 1:trialsPerRun
        ...
        %%%%%%%%% Make KbCheck ignore subsequent triggers %%%%%%%%% 
		...
		while 1
			...
			Screen(w, 'DrawText',tline,x0-180,y0-160+lCounter*45,[255]);
			lCounter	= lCounter + 1;
		end
		fclose(textfid);
		...
		while ( GetSecs - responseStart ) < questDur
			...
			%--------------------------SEE NOTE 2-----------------------------%
			%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			button						= intersect([30:39], find(keyCode));   
			if(RT(trial,1) == 0) & ~isempty(button)
				RT(trial,1)				= GetSecs - responseStart;
                temp                    = KbName(button);
				key(trial,1)			= str2num(temp(1));
			end
		end

		%%%%%%%%% Save information in the event of a crash %%%%%%%%%
		cd(behavdir);
		save([subjID '.tom_localizer.' num2str(run) '.mat'],'subjID','run','design','items','key','RT');
	end
catch exception
	ShowCursor;
	sca
...
%% Contrast
    ...
    onsets = [7 20 33 46 59 72 85 98 111 124]; % scan #
    
    con_info(1).name = 'belief vs photo';
    con_info(1).vals = [1,-1];
    
    for i=1:2 %num of conditions
        spm_inputs(i).name = condnames{i};
        spm_inputs(i).ons = onsets(find(design==i));
        spm_inputs(i).dur = [7;7;7;7;7]; % in scans
    end
...
try
	...
	%Previous version of save: save([subjID '.tom_localizer.' num2str(run) '.mat'],'subjID','run','design','items','key','RT','responses','experimentDuration','ips');
	    save([subjID '.tom_localizer.' num2str(run) '.mat'], 'RT','con_info','design','experimentDuration','ips','items','key','responses','run','spm_inputs','subjID');
    ShowCursor;
    cd ..;
catch exception
	sca
