...
%fname: output filename
	...
	output_labels={};
	recode_labels=zeros(length(dataset),1);
	output_labels_recode=zeros(length(dataset),1);
	all_softscores=[];
	...
	%for each training/testing set: fit the svm model and then test on the left-out one
		svmmodel = fitcsvm(training,classlabels,'KernelFunction','rbf','Standardize',true,'ClassNames',cnames);
	...
	class2_out = output_labels_recode(class1);
	correct_1 = find(class1_out==0); %out of those, which match their original class?
	correct_2 = find(class2_out==1);
	disp(['Class ' cnames{1} ' correct classification (%): ' num2str(correct_1/length(class1))]);
	disp(['Class ' cnames{2} ' correct classification (%): ' num2str(correct_2/length(class2))]);
