...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioral'));
            behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
...
%                 end
            ...
            %---------------------------------------------------------------------------
            for k=1:num_runs
                load(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioral',parafiles{k}),'con_info')
                if exist('con_info')
...
%         try
            img=fullfile(directory,img(1).name);
