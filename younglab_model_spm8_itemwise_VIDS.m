function younglab_model_spm8_itemwise (varargin)
% THIS VERSION modified to a) produce itemwise beta files and b) always find unsmoothed data.
% This is useful for modeling pre-RSA.
% It also assumes that: your study is under /mnt/englewood/data; that you already have
% skull_strip_masks; and that you have itemwise inputs in your behavioural .mats
% in the variable spm_inputs_itemwise.
%
% younglab_model_spm8
%
...
% 'spm_inputs_itemwise' in an individual mat file for every run that is to be
...
%       spm_inputs_itemwise is a scructure with fields name, ons, and dur
%
...
%                     load(behavdata_thistask(i).name,'spm_inputs_itemwise');
%                     for j=1:length(spm_inputs_itemwise)
%                         if length(spm_inputs_itemwise(j).ons) == 1
%                             % what do you do if there is only one instance of a
%                             % condition?
%                         else
%                             for k=1:(length(spm_inputs_itemwise(j).ons)-1)
%                                 thisPeriod = spm_inputs_itemwise(j).ons(k+1) - spm_inputs_itemwise(j).ons(k);
...
tic;
[ErrorLog,SPM]=model(study,subject,tasks,boldirs,src_data_flag,TR,RT_flag,implicit_flag,filter_frequency,maskthresh,smooth_src_data_flag,units_flag);
...
%-------------------------------------------------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ErrorLog,SPM]=model(study,subject,tasks,boldirs,src_data_flag,TR,RT_flag,implicit_flag,filter_frequency,maskthresh,smooth_src_data_flag,units_flag)
global EXPERIMENT_ROOT_DIR;
global skip_art;
global defaults;
global inStruct;
...
for subj = 1:length(subject)
    ...
    for task = 1:length(tasks{subj})
        ...
        clear spm_inputs_itemwise con_info ips scans; SPM=[];
        cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural/behavioural_VIDS'));
        behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
        ...
        results_string = [tasks{subj}{task} '_results_itemwise_' src_data_flag];
        ...
        cd(results_string);
        rS1 = pwd;
        percRetained = freqAnalyze_itemwise_VIDS(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
        set(0,'DefaultFigureVisible','Off');
        ...
        for run = 1:num_runs
            ...
            clear RT
            % try
            load(parafiles{run},'spm_inputs_itemwise','con_info','ips','user_regressors','RT','design','item_vector');
            % catch
            %     load(parafiles{run},'spm_inputs_itemwise','con_info','ips','user_regressors','RT','design');
            % end
            if ~exist('spm_inputs_itemwise') | ~exist('ips')
                fprintf('Failed to load behavioural data for subject %s task `%s`\n',subject{subj}, tasks{subj}{task});
                fprintf('\t-variables "spm_inputs_itemwise" and "ips" required (and "con_info" is handy)\n');
            ...
            SPM.xBF.UNITS      = units_flag;        % OPTIONS: 'scans'|'secs' for onsets
            ...
            if max(horzcat(spm_inputs_itemwise(:).ons))/TR > ips  % saxelab uses vertcat but horzcat should do the same thing
                warning('\n----------------------------------------\nYour maximum onset: %.0f is greater than your ips (%.0f)!\n',max(vertcat(spm_inputs_itemwise(:).ons)),ips);
                ...
                elseif strcmpi(onset_resp,'c')
                    ...
                    for onsTask = 1:length(spm_inputs_itemwise)
                        spm_inputs_itemwise(onsTask).ons = spm_inputs_itemwise(onsTask).ons/2+1;
                    end
                end
            end
            for cond = 1:length(spm_inputs_itemwise)
                ...
                SPM.Sess(run).U(cond).name   = {spm_inputs_itemwise(cond).name};   % string from spm_inputs_itemwise.name
                SPM.Sess(run).U(cond).dur    = spm_inputs_itemwise(cond).dur;      % duration in scans from spm_inputs_itemwise.dur
            ...
            if RT_flag==1
                ...
                for i=1:length(spm_inputs_itemwise)
                    tp   = [tp spm_inputs_itemwise(i).ons'];
                    tdur = [tdur spm_inputs_itemwise(i).dur'];
                end
                ...
                %                         disp('    -# non-fixation onsets in RT, design, and spm_inputs_itemwise do not match')
            ...
            clear spm_inputs_itemwise ips user_regressors
        end % run/acq loop
