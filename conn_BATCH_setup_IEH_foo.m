function filename = conn_BATCH_setup_IEH(varargin)
% =================================================
...
%=================================================
	...
	clear BATCH;
	BATCH.filename=fullfile(pwd,save_filename);
	...
	if mask_subject 
		...
		BATCH.Setup.voxelmaskfile='/software/spm12/toolbox/OldNorm/EPI.nii'; 
	end
...
%         for sess=1:6
%             if length(BATCH.Setup.functionals{s}{sess})~=167
...
%     end
	...
	if default_roi
		...
		BATCH.Setup.rois=make_roi_struct_PSYCH(study,subjIDs,length(sessions{1}),anatomical,{'DMPFC'}); %
	end
