function permute_test_roi(rootdir,study,resdir,subnums,bmatrix,iter,roi,findtag,starting_iter)
...
% - findtag: identifier for categorical RSA results i.e., the H0 model
    ...
    for it = starting_iter:iter
    	if(it > 1 && it == starting_iter)
    		cd(fullfile(rootdir,study,'results'));
        	load(['permute_test_' roi '_' bmatrix '.mat'],'H','P','RDIFF2');
        end
        disp(['ITERATION ' num2str(it)]);
		cd(fullfile(rootdir,study,'behavioural_all'));
		load(['behav_matrix_' bmatrix '.mat']);
		bmat=behav_matrix;
        ...
        delete(fullfile(rootdir,study,'behavioural_all',bname));
