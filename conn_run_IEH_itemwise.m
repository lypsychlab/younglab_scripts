...
diary(fullfile(rootdir,study,'logs',['conn_run_IEH_itemwise_' date '.txt']));
fname='IEHFMRI_ROI_allrois_128Hz';
...
conditions.names={};
for c=1:120
    conditions.names{end+1}=num2str(c);
end
...
conditions.onsets=cell(120,length(subjs),8);
conditions.durations=cell(120,length(subjs),8);
...
for sub=1:length(subjs)
    for runnum=1:8 %this corresponds to sessions{sub} index as well
        ...
        L = length(matname.spm_inputs_itemwise); %length BEFORE adding rest
%         matname.spm_inputs_itemwise(L+1).name='rest';
        
%         onset_list=[167];rest_start=[1];
%         for on=1:L
%             onset_list=[onset_list matname.spm_inputs_itemwise(on).ons/TR];
%             rest_start=[rest_start matname.spm_inputs_itemwise(on).ons+(60/TR)]; %this will be the vector of rest onsets
%         end
...
%         end
%         matname.spm_inputs_itemwise(L+1).ons=rest_start;
%         matname.spm_inputs_itemwise(L+1).dur=rest_durations;
        
        multiplier=(runnum-1);
        set_bounds=[(1+multiplier*15):(15+multiplier*15)];
        for cond=1:120
            if ismember(cond,set_bounds)
                conditions.onsets{cond}{sub}{runnum}=matname.spm_inputs_itemwise(cond-multiplier*15).ons/TR;
                conditions.durations{cond}{sub}{runnum}=matname.spm_inputs_itemwise(cond-multiplier*15).dur/TR;
            else
               conditions.onsets{cond}{sub}{runnum}=0;
               conditions.durations{cond}{sub}{runnum}=0; 
            end
        end
%         conditions.onsets{121}{sub}{runnum}=matname.spm_inputs_itemwise(16).ons/TR;
%         conditions.durations{121}{sub}{runnum}=matname.spm_inputs_itemwise(16).dur/TR;
%         spm_inputs_itemwise=matname.spm_inputs_itemwise;
%         save([subjs{sub} '.ieh.' num2str(runnum) '.mat'],'spm_inputs_itemwise','-append');
%         clear matname;
    end
end
%fill in the zeros
...
%         for cond=1:120
%             cond
%             if isempty(conditions.onsets{cond}{sub}{runnum})
%                 conditions.onsets{cond}{sub}{runnum}=0;
%             end
%         end
%     end
