...
%                     try
                ...
                split_files_dir = ['/data/younglw/lab/' study '/derivs/fmriprep/' subj_bids{subj} '/func/' boldfiles{subj}{task} '_run_' num2str(boldirs{subj}{task}(run))];

                if 7 == exist(split_files_dir,'dir')
                  fprintf('Unzipped directory already exists')
                  files{run} = alek_get(split_files_dir,'smooth*nii');
                else
                  run_zip = alek_get(sprintf('%s/%s/derivs/fmriprep/%s/func',EXPERIMENT_ROOT_DIR,study,subj_bids{subj}),strcat(subj_bids{subj},'*',boldfiles{subj}{task},sprintf('*%.2d*',boldirs{subj}{task}(run)),filter));
