function searchlight_all_regress_2(study,subj_tag,resdir,sub_nums,conditions,sph,b1,b2,outtag)
...
% - [searchlight images]: .nii files prepended with "RSA_searchlight_regress", in results dir
	...
	for subj=1:length(subjIDs) %grabbing beta images
		...
		load(fullfile(rootdir, study, 'behavioural',['behav_matrix_' b1 '.mat']));
        disp(fullfile(rootdir, study, 'behavioural',['behav_matrix_' b1 '.mat']));
		behav_1=behav_matrix; clear behav_matrix;
		load(fullfile(rootdir, study, 'behavioural',['behav_matrix_' b2 '.mat']));
        disp(fullfile(rootdir, study, 'behavioural',['behav_matrix_' b2 '.mat']));
	    ...
	    corrs  = zeros(length(greymattermask2),3); 
	    for i = 1:length(greymattermask2)% for each voxel
	        ...
	        if length(goodrows) > 9 % if there are at least 10 good voxels in this sphere
	            ...
	            predictors = horzcat(ones(length(behav_1),1),behav_1,behav_2);
                ...
                corrs(i,3)=weights(1); % constant

	        end
	    end
	    clear temp simmat behav_1 behav_2 weights predictors
	    ...
	    template.fname = ['RSA_searchlight_regress_' b1 outtag '.img']; spm_write_vol(template,corrmap_1);
	   	...
	   	template.fname = ['RSA_searchlight_regress_const_' outtag '.img']; spm_write_vol(template,corrmap_3);
