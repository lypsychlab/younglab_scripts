function younglab_preproc_temporal_spm12_pleiades_harvard (varargin)
%
...
for subj_index=1:length(subject)
    ...
    % try
        ...
        keyboard
        slice_times=get_slicetimes(study, subjID,func_runs,func_images);
        keyboard
        fprintf ('===============================\n');
        fprintf ('Slice-timing correcting subject %s\n',subjID);
        fprintf ('===============================\n');
        slicetiming(study,subjID,func_images, slice_times,func_runs);
        fprintf ('=====================================\n');
...
function [slice_times] = get_slicetimes(study,subjID,func_runs,func_images)
global EXPERIMENT_ROOT_DIR;
slice_times = [];
for run=1:length(func_runs);
    cd(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID, 'bold', char(func_runs(run))));
    func_imageNames=dir('*.hdr');func_imageNames={func_imageNames.name}.';
    func_image=1;
    image_name= char(func_imageNames(1));
    image_key = image_name(3:14);
    image_key(7)='';
    dicom_key= strrep(image_key,'-','.');
    cd(fullfile(EXPERIMENT_ROOT_DIR,  study, subjID,'dicom'));
    dicom_header = strcat('*INVESTIGATORS_YOUNG',dicom_key,'*');
    dicom_name=dir(dicom_header);dicom_name={dicom_name.name}.';
    metadata = dicominfo(char(dicom_name));
    slice_times(run,:) = metadata.Private_0019_1029';
    end;    
end % function get_slicetimes



%=======================================================================
function slicetiming(study,subjID,func_images,slice_times,func_runs)
global EXPERIMENT_ROOT_DIR;
%%
...
for run=6:2:12
    matlabbatch{1}.spm.temporal.st.scans = {
                                            {
                                            strcat(cellstr(func_images{run}),',1')
                                            }
                                            }';
    matlabbatch{1}.spm.temporal.st.nslices = length(slice_times);
    matlabbatch{1}.spm.temporal.st.tr = 2;
    matlabbatch{1}.spm.temporal.st.ta = 2-(2/length(slice_times));
    matlabbatch{1}.spm.temporal.st.so = slice_times(run,:);
