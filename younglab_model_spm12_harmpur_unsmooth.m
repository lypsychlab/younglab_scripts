function younglab_model_spm12_harmpur_unsmoothed(varargin)
% 
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            cd(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural_harm_pur'));
            behavdata_thistask = dir(sprintf('%s.%s.*.mat',subject{subj},tasks{subj}{task}));
            ...
            results_string = [tasks{subj}{task} '_results_harmpur_unsmooth' src_data_flag];
...
%                     end
            ...
            %---------------------------------------------------------------------------
            for k=1:num_runs
                load(fullfile(EXPERIMENT_ROOT_DIR,study,'behavioural_harm_pur',parafiles{k}),'con_info')
                if exist('con_info')
