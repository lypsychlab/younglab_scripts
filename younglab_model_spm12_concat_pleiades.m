function younglab_model_spm12_concat_pleiades(varargin)
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            results_string = [tasks{subj}{task} '_results_concat_' src_data_flag];
...
%                 SPM.xBF.UNITS      = 'secs';        % OPTIONS: 'scans'|'secs' for onsets
				...
				% if (max(vertcat(spm_inputs(:).ons))/TR) > ips % saxelab uses vertcat but horzcat should do the same thing
    ...
    %                         spm_inputs(onsTask).dur = spm_inputs(onsTask).dur/TR;
                ...
                for cond = 1:length(spm_inputs)
                    ...
                    % image is 1.
                    SPM.Sess(1).U(cond).ons    = [SPM.Sess(1).U(cond).ons; (spm_inputs(cond).ons - 1)+((run-1)*ips)];  % onsets in scans. 
                    SPM.Sess(1).U(cond).name{1}   = spm_inputs(cond).name{1};   % string from spm_inputs.name
                    % SPM.Sess(1).U(cond).name(1)   = {spm_inputs(cond).name};   % string from spm_inputs.name
                    SPM.Sess(1).U(cond).dur    = [SPM.Sess(1).U(cond).dur spm_inputs(cond).dur];      % duration in scans from spm_inputs.dur
                    % SPM.Sess(1).U(cond).dur    = [SPM.Sess(1).U(cond).dur; spm_inputs(cond).dur];      % duration in scans from spm_inputs.dur
