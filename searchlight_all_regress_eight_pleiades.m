function searchlight_all_regress_eight_pleiades(study,subj_tag,resdir,sub_nums,conditions,sph,b1,b2,b3,b4,b5,b6,b7,b8,outtag)
...
% - [searchlight images]: .nii files prepended with "RSA_searchlight_regress", in results dir
	...
	for subj=1:length(subjIDs) %grabbing beta images
		...
		load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b1 '.mat']));
        disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b1 '.mat']));
		behav_1=behav_matrix1; clear behav_matrix1;
		load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b2 '.mat']));
        disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b2 '.mat']));
		behav_2=behav_matrix2; clear behav_matrix2;
		load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b3 '.mat']));
		disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b3 '.mat']));
        behav_3=behav_matrix; clear behav_matrix;
		load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b4 '.mat']));
		disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b4 '.mat']));
        behav_4=behav_matrix; clear behav_matrix;
        load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b5 '.mat']));
		disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b5 '.mat']));
        behav_5=behav_matrix; clear behav_matrix;
        load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b6 '.mat']));
		disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b6 '.mat']));
        behav_6=behav_matrix; clear behav_matrix;
        load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b7 '.mat']));
		disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b7 '.mat']));
        behav_7=behav_matrix; clear behav_matrix;
        load(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b8 '.mat']));
		disp(fullfile(rootdir, study, 'behavioural_pp',['behav_matrix_' subjIDs{subj} '_' b8 '.mat']));
	    ...
	    bigmat = zeros(length(greymattermask),triangle);
	    corrs  = zeros(length(greymattermask),9); 
	    for i = 1:length(greymattermask)% for each voxel
	    	disp(['Voxel ' num2str(i)]);
	        sphere      = repmat(voxel_order(greymattermask(i),:),length(coords),1) + coords; 
	        ...
	        if length(goodrows) > 9 % if there are at least 10 good voxels in this sphere
                ...
                corrs(i,6)=weights(7); % b6
                corrs(i,7)=weights(8); % b7
	            corrs(i,8)=weights(9); % b8

                corrs(i,9)=weights(1); % constant
        ...
        corrmap_7  = zeros(size(Y(:,:,:,1)));
        corrmap_8  = zeros(size(Y(:,:,:,1)));
        corrmap_9  = zeros(size(Y(:,:,:,1)));

        clear Y;

	    for i=1:length(greymattermask) 
	            corrmap_1(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,1);
				corrmap_2(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,2);
				corrmap_3(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,3);
				corrmap_4(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,4);
				corrmap_5(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,5);
                corrmap_6(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,6);
                corrmap_7(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,7);
                corrmap_8(voxel_order(greymattermask(i),1),...
	                    voxel_order(greymattermask(i),2),...
	                    voxel_order(greymattermask(i),3)) = corrs(i,8);
                corrmap_9(voxel_order(greymattermask(i),1),...
                        voxel_order(greymattermask(i),2),...
                        voxel_order(greymattermask(i),3)) = corrs(i,9);
	    end
        ...
        template.fname = ['RSA_searchlight_regress_' b6 outtag '.img']; spm_write_vol(template,corrmap_6);
        template.fname = ['RSA_searchlight_regress_' b7 outtag '.img']; spm_write_vol(template,corrmap_7);
        template.fname = ['RSA_searchlight_regress_' b8 outtag '.img']; spm_write_vol(template,corrmap_8);
	   	template.fname = ['RSA_searchlight_regress_const_' outtag '.img']; spm_write_vol(template,corrmap_9);
