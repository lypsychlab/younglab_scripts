function younglab_dicom_convert_3t1_josh (varargin)
% 
% MODIFICATIONS FOR 3T1 SCANNER:
% - assumes that the MPRAGE scan is last run of the structural files (i.e. highest run number).
% 
...
% 2) THe MPRAGE scan is the last run of the structural runs.
% 3) Skips motion-corrected runs as a default (see 'pace' in optional arguments)
% 
...
%     delete('*')
    ...
    %for i=1:length(dirname)
        %keyboard;
        %movefile([dirname(i).name '/' '*'], strcat(subj_dir,'/dicom'));
    ...
    sfiles = dir([dicom_dir '/s*']);
    struc_run = zeros(1,length(sfiles));
    for i = 1:length(sfiles)
        struc_run_num = str2double(sfiles(i).name(6:7));
        struc_run(i)=struc_run_num;
    end
    mprage_run = max(struc_run); %assumes largest run number in structural files is MPRAGE
    mprage_string = strcat('/s*',num2str(mprage_run),'*nii');
    mprage_files = dir([dicom_dir mprage_string]);
%     sfiles = dir([dicom_dir '/s*img']);
    sizefiles=zeros(1,length(mprage_files));
    for i=1:length(mprage_files)
        temp=dir(fullfile(dicom_dir,mprage_files(i).name));
        sizefiles(i)=temp.bytes;
    end
    [val, loc] = max(sizefiles); % this was used in saxelab, which picks out MPRAGE 
    movefile(fullfile(dicom_dir,[mprage_files(loc).name(1:end-3) '*']),anatdir);
    ...
    %Remove run 006 (vNav) from bold folder, headers in dicoms -> spm_converter to mistakenly prepend files with "f" prefix
    vNav_dir = fullfile(subj_dir,'bold','022');
    movefile(vNav_dir, fullfile(subj_dir,'3danat'));
