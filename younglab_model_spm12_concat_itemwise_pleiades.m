function younglab_model_spm12_concat_itemwise_pleiades(varargin)
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            results_string = [tasks{subj}{task} '_results_concat_itemwise_' src_data_flag];
            ...
            cd(results_string);
            rS1 = pwd;
			percRetained = freqAnalyze_itemwise_pleiades(study, subject{subj}, tasks{subj}{task}, filter_frequency, rS1);
            set(0,'DefaultFigureVisible','Off');
...
%                 SPM.xBF.UNITS      = 'secs';        % OPTIONS: 'scans'|'secs' for onsets
                ...
                for cond = 1:length(spm_inputs_itemwise)
                    disp(['Filling inputs for condition ' spm_inputs_itemwise(cond).name '...']); 
                    ...
                    % image is 1.
                    indx=cond+((run-1)*length(spm_inputs_itemwise));
                    disp(['indx: ' num2str(indx)]);
                    SPM.Sess(1).U(indx).ons    = [(spm_inputs_itemwise(cond).ons - 1)+((run-1)*ips)];  % onsets in scans. 
                    SPM.Sess(1).U(indx).name   = {spm_inputs_itemwise(cond).name};   % string from spm_inputs.name
                    SPM.Sess(1).U(indx).dur    = [spm_inputs_itemwise(cond).dur];      % duration in scans from spm_inputs.dur
                    SPM.Sess(1).U(indx).P(1).name='none';
                end
...
%                     end
                ...
                disp('Creating movement regressors...')
                num_regressors = 0;
                if exist('user_regressors')
                    if run==1
                        temp=SPM.Sess(1).C.C; 
                        ncols=size(temp,2); % number of preexisting regressors
                        temp=[temp zeros(size(temp,1),length(user_regressors))]; %initialize empty zeros for new regs
                        SPM.Sess(1).C.C=temp;
                    end
...
%                 end
            ...
            %             if k==1, con_vals{end+1} = []; end
            %             if exist('RT') && RT_flag==1
            %                 con_vals{end+1} = [con_vals{end+1} con_info(contrast).vals 0];
            %             else
            %                 con_vals{end+1} = [con_vals{end+1} con_info(contrast).vals];
            %             end
            %             con_name{end+1} = con_info(contrast).name;
            ...
            %                 con_vals{end+1}=[con_vals{contrast},zeros(1,n)]; %pad with zeros
            %             end
            %         end
            %     end
            % end
            % add contrasts for run regressors:
%             conlength=length(con_name);
%             cv=eye(6);
%             for k=1:num_runs
...
%             for contrast=1:length(con_name)
