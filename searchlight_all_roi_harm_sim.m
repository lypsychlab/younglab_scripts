function [corrs,rp] = searchlight_all_roi_harm_sim(study,subj_tag,simfolder,sub_nums,conditions,behav_tag,roiname,outtag)
% searchlight_all_roi(study,sub_nums,conditions,sph,b1,b2,b3,b4):
% - performs searchlight RSA within a given ROI.
% 
%
% Parameters:
...
% 	    [Y,XYZ]   = spm_read_vols(subimg);clear betadir betafiles XYZ %read volumes
        cd(fullfile(rootdir,study,subjIDs{subj},'results/simulation',simfolder));
        load([subjIDs{subj} '.mat']);
...
%         goodrows = find(isnan(spherebetas(:,1)) == 0);
        simmat      = corrcoef(R);% item similarities for this subject
        temp        = tril(simmat,-1); 
        bigmat = temp(temp~=0)';% we now have a triangle x 1 matrix

        [temp,rp]=corrcoef( behav_matrix , bigmat' );
