...
evalin('base','xY.xyz     = spm_mip_ui(''GetCoords'',findobj(spm_figure(''FindWin'',''Graphics''),''Tag'',''hMIPax''));');
evalin('base',['xY.name   = ''' roi_name ''';']);
evalin('base','xY.Ic      = 0;');
evalin('base','xY.Sess    = 1;');
evalin('base','xY.def     = ''sphere'';');
evalin('base',['xY.spec   = ' num2str(r) ';']);
evalin('base','[Y,xY] = spm_regions(xSPM,SPM,hReg,xY);'); %This closed the window.
