function younglab_model_spm12_parametric_ieh(varargin)
...
diary(fullfile(rootdir,study,'logs',['younglab_model_spm12_parametric_ieh_' date '.txt']));
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            results_string = [tasks{subj}{task} '_resultsNEW_autocon_param_' src_data_flag];
...
% 				end
                for cond = 1:length(spm_inputs)
                    ...
                    SPM.Sess(run).U(cond).P.name = 'WTH';                    % 'none' | 'time' | 'other'
                    SPM.Sess(run).U(cond).P.h = 1;              % order of polynomial expansion
                    if cond<5
                        SPM.Sess(run).U(cond).P.P = param{cond}{1}';     % vector as long as current U's ons vector
                    else
                        SPM.Sess(run).U(cond).P.P = repmat([NaN],length(spm_inputs(cond).ons),1);
                    end
