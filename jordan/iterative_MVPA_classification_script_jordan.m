...
input_id = {'YOU_TRAG_03','YOU_TRAG_04','YOU_TRAG_05', 'YOU_TRAG_06','YOU_TRAG_08','YOU_TRAG_09',...
'YOU_TRAG_10','YOU_TRAG_11','YOU_TRAG_12','YOU_TRAG_13','YOU_TRAG_15','YOU_TRAG_16', 'YOU_TRAG_19', 'YOU_TRAG_20',...
'YOU_TRAG_21', 'YOU_TRAG_22', 'YOU_TRAG_23', 'YOU_TRAG_24', 'YOU_TRAG_25','YOU_TRAG_26','YOU_TRAG_27','YOU_TRAG_28',...
'YOU_TRAG_29',};
input_runs = {6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6};
num_conds = 3;

study_path = '/home/younglw/lab/TRAG/';
model_path = '/results/FPMLocal_CombinedDOA_results_normed';
outputfolder = '/home/younglw/lab/TRAG/FPM_MVPA';
outputname = 'FPM_ROI_MVPA';

condition_names = {'A_FAC_ALL','B_PRF_ALL', 'C_MRL_ALL'}; %These should match the order that they are listed in spm_input in the behavioural files.
roiFiles = {'RTPJ', 'LTPJ', 'DMPFC', 'VMPFC', 'PC'}; %it will wildcard either side of this, so make sure
%that these uniquely identify a ROI. It is looking for an image file.
contrast_names = {'F-P', 'F-M', 'M-P'};
...
experiments=struct(...
    'name','TRAG',... % study name you %%%%need to change this
...
%                             end
                        ...
                        elseif strcmp('Y', preproc_runwise)
                                ...
                                elseif strcmp('Y', preproc_cocktail)
                                    this_vol(:,ismembc(preproc_ind,oo)) = voxel_to_preprocess - mean(mean(voxel_to_preprocess)); %cocktail
                                end
                        end
                    end
    ...
    for nn = 1:numel(masked_vols), %going through each mask
        ...
        if ~isempty(masked_vols{nn}) %skip empty masks.
            ...
            for oo = 1:(numel(tester_set(:,1))/2); %going through each tester set. After it is halfway through they become repeats.
                ...
                for pp = 1:num_conds
                    testmeaned_conditions(:,pp) = mean(tester_voxels(:,ismember(preproc_ind_fs(1:numel(tester_voxels(1,:))),pp)),2);         
                    trainmeaned_conditions(:,pp) = mean(training_voxels(:,ismember(preproc_ind_fs(1:numel(training_voxels(1,:))),pp)),2);
                end
...
%                 results.ROI(nn).contrasts(zz).mean_between
            ...
            for zz = 1:numel(contrasts.contrasts(:,1)) 
            results.ROI(nn).contrasts(zz).acc = [results.ROI(nn).contrasts(zz).acc;0];
            end
            results.ROI(nn).within(mm,:) = zeros(1,numel(contrast_names));
        end    
    end %nn
...
output_header = cell(2,numel(contrast_names)*numel(roiFiles));
output_header(1,:) = repmat(contrast_names,1,numel(roiFiles));
tmp = repmat(roiFiles,1,numel(contrast_names));
% output_header(2,:) = repelem(roiFiles,numel(contrast_names));
output_ids(:,1) = input_id;

output = zeros(numel(input_id), numel(contrast_names)*numel(roiFiles));

for nn = 1:numel(masked_vols) %ROI
    for zz = 1:numel(contrast_names) %contrast
        output_header(2,(zz+(numel(contrast_names))*(nn-1))) = roiFiles(nn);
        output(:,(zz+(numel(contrast_names))*(nn-1))) = results.ROI(nn).contrasts(zz).acc;
    end
end
% results.output={};
%Non-parametric
...
cd(outputfolder);
save(sprintf([outputname preprocname '_header' '.mat']), 'output_header');
save(sprintf([outputname preprocname '_IDs' '.mat']), 'output_ids');
save(sprintf([outputname preprocname '_raw' '.mat']), 'output');
