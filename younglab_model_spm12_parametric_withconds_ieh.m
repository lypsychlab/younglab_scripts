function younglab_model_spm12_parametric_withconds_ieh(varargin)
...
tic;
dname = fullfile(EXPERIMENT_ROOT_DIR,study,'logs',['younglab_model_spm12_parametric_withconds_' date '.txt']);
diary(dname);
disp(['Writing log to ' get(0,'DiaryFile')]);
...
%     try
        ...
        for task = 1:length(tasks{subj})
            ...
            results_string = [tasks{subj}{task} '_resultsNEW_autocon_param_concat_withconds_scen_' src_data_flag];
            ...
            for paramcond=1:6
                SPM.Sess(1).U(paramcond).ons=[];
                SPM.Sess(1).U(paramcond).dur=[];
                if paramcond<5
                    for modulator=1:1
                        SPM.Sess(1).U(paramcond).P(modulator).name='';
                        SPM.Sess(1).U(paramcond).P(modulator).P=[];
                        SPM.Sess(1).U(paramcond).P(modulator).h=[];
                    end
                end
            end
            for run = 1:num_runs
...
% 				end
                ...
                for cond = 1:length(spm_inputs)
                    ...
                    % image is 1.
                    SPM.Sess(1).U(cond).ons =[SPM.Sess(1).U(cond).ons (spm_inputs(cond).ons - 1)];
                    SPM.Sess(1).U(cond).name   = {spm_inputs(cond).name};
                    SPM.Sess(1).U(cond).dur=[SPM.Sess(1).U(cond).dur; (spm_inputs(cond).dur)];
                    if ismember(cond,[1 2 4])
                        %fill in DV values
                        disp(['Filling in parametric modulator for condition ' num2str(cond)]);
                        SPM.Sess(1).U(cond).P(1).P = [SPM.Sess(1).U(cond).P(1).P param{cond}{1}];
                        SPM.Sess(1).U(cond).P(1).name = 'WTH';                    % 'none' | 'time' | 'other'
                        SPM.Sess(1).U(cond).P(1).h = 1;              % order of polynomial expansion
                    else 
                        % 0 where there are no DV values
                        disp(['Skipping parametric modulator for condition ' num2str(cond)]);
                        SPM.Sess(1).U(cond).P(1).name = 'none';
                    end
                end
...
%                     end
            ...
            for thisparam=1:4
                SPM.Sess(1).U(thisparam).P(1).P=SPM.Sess(1).U(thisparam).P(1).P';
            end

            % demean ONLY the DV parametric modulator
            % (turning off spm_orth calls requires doing this by hand)
            for paramcond=[1 2 4]
                disp(['Demeaning parametric modulator ' SPM.Sess(1).U(paramcond).P(1).name '...']);
                dv = SPM.Sess(1).U(paramcond).P(1).P;
                ...
                SPM.Sess(1).U(paramcond).P(1).P=dv_demean;
            end
