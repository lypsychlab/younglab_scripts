function roi_picker_BIDS(t,k,r,c,roi_name,roi_xyz,subjects,study,res_dir)
...
% e.g.:
% roi_picker(.001,5,9,1,'RTPJ','[0; 0; 0]',{'/mindhive/saxelab/CUES3/SAX_cues3_05'},'CUES3','results/cues3_results_normed');
%
% Written by Alek Chakroff, November 2009
% Edited by Emily Wasserman, 2016
% Note: use with SPM8 & Matlab 2012 - Emily

% added by KJ for running mat2nii if scripts folder not added, 5/8/19
...
spm fmri;
run_it(t,k,r,c,roi_name,roi_xyz,subjects,study,1,notes,res_dir);
end

function run_it(t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir)
if i>length(subjects) % just finished last subject
    ...
    if ismember(roi_name,TOM_rois) & ischar(roi_xyz)
        roifile=load(fullfile('/data/younglw/lab/roi_library/functional',[roi_name '_xyz.mat']));
    ...
    % Overlay on T1 image
    try
        bids_id = strcat('sub-',subjects{i}(end-1:end));
        imgdir = dir(fullfile('/data/younglw/lab',study,'derivatives/fmriprep',bids_id,'anat','*MNI*preproc_T1w.nii'));
        if length(imgdir)==0
            imgdir = dir(fullfile(subjects{i},'3danat','ws*.nii'));
        end
        evalin('base',['spm_sections(xSPM,findobj(spm_figure(''FindWin'',''Interactive''),''Tag'',''hReg''),''' fullfile('/data/younglw/lab',study,'derivatives/fmriprep',bids_id,'anat',imgdir(1).name) ''');']);
    catch
    ...
    uicontrol('Style','pushbutton',       'Units','normalized',...
        ...
        'BackgroundColor',[.9 .5 .9],'Callback',{@back_one,t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir});
    % bad button
    uicontrol('Style','pushbutton',       'Units','normalized',...
        'Position',[.33 .59 .13 .1],'String','None found',...
        'BackgroundColor','red','Callback',{@bad_one,t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir});
    % good button
    uicontrol('Style','pushbutton',       'Units','normalized',...
        'Position',[.47 .59 .13 .1],'String','Continue',...
        'BackgroundColor','green','Callback',{@good_one,t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir});
end
end

function good_one(hObject,eventdata,t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir)
...
if ~exist(fullfile(subjects{i},'roi'),'dir'); % modified by KJ, 5/8/19, creates roi folder in correct location
    mkdir(fullfile(subjects{i},'roi'));
    if ~exist(fullfile(subjects{i},'roi'),'dir')
...
try
    mat2nii( fullfile(subjects{1},res_dir,'spmT_0001.img'),fullfile(subjects{i},'roi',['ROI_' roi_name '_' task '_' num2str(c) '_' date '_xyz.mat']) );
catch
    mat2nii( fullfile(subjects{1},res_dir,'spmT_0001.nii'),fullfile(subjects{i},'roi',['ROI_' roi_name '_' task '_' num2str(c) '_' date '_xyz.mat']) );
end

% kevin
% keyboard;
% try
%     mat2nii( fullfile(subjects{i},res_dir,'spmT_0001.img'),fullfile(subjects{i},'roi',['ROI_' roi_name '_' task '_' num2str(c) '_' date '_xyz.mat']) );
% catch
%     mat2nii( fullfile(subjects{i},res_dir,'spmT_0001.nii'),fullfile(subjects{i},'roi',['ROI_' roi_name '_' task '_' num2str(c) '_' date '_xyz.mat']) );
% end
% update log
...
% go to next subject
run_it(t,k,r,c,roi_name,roi_xyz,subjects,study,i+1,notes,res_dir);
end

function bad_one(hObject,eventdata,t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir)
...
function back_one(hObject,eventdata,t,k,r,c,roi_name,roi_xyz,subjects,study,i,notes,res_dir)
if i>1
    % update log
    temp         = strread(res_dir,'%s','delimiter','/');
    task         = temp{length(temp)};
