function mat2nii(varargin)
%Given a list of .tal files - or, if no argument is given, the user
% selects a list - mm2nii creates an ROI in nii format.

if nargin==2
    ROI        = varargin{1};
    roi_fulloc = varargin{2};
else
    ROI        = spm_select(1,'nii','Choose a Template image','',pwd,'.*',1);
...
out = fullfile(ROI_path, [ROI_name '.nii']); v.fname = out;
spm_write_vol(v,Y_roital);
fprintf(['Wrote: ' fullfile(ROI_path, [ROI_name '.nii']) '\n']);
