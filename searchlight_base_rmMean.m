function searchlight_base_rmMean(rootdir,study,subj_tag,resdir,sub_nums,cond_in,sph,B_in,outtag)
% searchlight_base_rmMean(rootdir,study,subj_tag,resdir,sub_nums,conditions,sph,B_in,outtag):
	...
	for subj=1:length(subjIDs) %grabbing beta images
	    ...
	    for i = 1:length(greymattermask2)% for each voxel
	        ...
	        if length(goodrows) > 9 % if there are at least 10 good voxels in this sphere
	        	% remove mean BOLD signal in this sphere from each beta
	        	voxMat = spherebetas(goodrows,:);
	        	voxMat = voxMat - repmat([mean(voxMat)],length(goodrows),1);

	            simmat      = corrcoef(voxMat);% item similarities for this subject
