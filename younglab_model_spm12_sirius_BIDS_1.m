...
%                     try
                ...
                run_zip = alek_get(sprintf('%s/%s/derivs/fmriprep/%s/func',EXPERIMENT_ROOT_DIR,study,subj_bids{subj}),strcat(subj_bids{subj},'*',tasks{subj}{task},sprintf('*%.2d*',boldirs{subj}{task}(run)),filter));
                unzipped = gunzip(run_zip);
                split_files_dir = ['/data/younglw/lab/' study '/derivs/fmriprep/' subj_bids{subj} '/func/run_' num2str(boldirs{subj}{task}(run))];
                try rmdir (split_files_dir, 's')
                catch
                  fprintf('There was no directory %d \n',split_files_dir)
                end
            ...
            %===========================================================================
                ...
                if implicit_flag == 1
                    ...
                    SPM.xM.I           = 0;
                    SPM.xM.VM          = spm_vol([fullfile(EXPERIMENT_ROOT_DIR,study,'derivs/fmriprep/',subj_bids{subj},'anat') '/skull_strip_mask.nii']);
...
function skullStripMaker(study, subj, maskthresh)
    ...
    global mask_over;
    global inStruct;
    directory = ['/data/younglw/lab/' study '/derivs/fmriprep/' subj '/anat/'];
    if ~exist([directory 'sub-*MNI*preproc.nii'],'file')
    ...
    elseif exist([directory 'sub-*MNI*preproc.nii'],'file')
      img = dir([directory 'sub-*MNI*preproc.nii']);
    else
      pwd1 = pwd;
      cd(directory);
      fprintf('Could not find proper anatomical file.')
      cd(pwd1);
    end

    if strcmp(inStruct.src_data_flag,'unnormed')|isempty(img)
        if ~exist([directory 'sub-*T1w_preproc.nii'],'file')
        ...
        elseif exist([directory 'sub-*T1w_preproc.nii'],'file')
          img = dir([directory 'sub-*T1w_preproc.nii']);
        elseif isempty(img)
            pwd1 = pwd;
            cd(directory);
