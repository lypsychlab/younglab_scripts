    ...
    elseif strcmp(type,'image')
        ...
        if auto_motion_fname && length(P{sess})<=1
            ...
            elseif motionFileType == 0
                M{sess}=[];
                for n1=0:5,
                    if ~isempty(dir(strprepend('rp_concat_',strprepend(-n1,fullfile(tmotion_dir,s)),'.txt'))),
                        M{sess} = load(strprepend('rp_concat_',strprepend(-n1,fullfile(tmotion_dir,s)),'.txt'));
                        break;
                    end
                end
