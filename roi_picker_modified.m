function roi_picker_modified(study,subj_tag,t,k,r,c,roi_name,roi_xyz,subjects,res_dir)
% roi_picker(t,k,r,c,roi_name,roi_xyz,subjects,res_dir):
% - pick ROIs per subject, given certain parameters, and generate output files
% to use subsequently in roi_batch.
% This is called on by roi_picker_gui.
%
% Parameters:
% - study: study name
% - subj_tag: prefix to subject names (e.g. 'YOU_XPECT')
% - t: threshold (float number)
% - k: cluster extent (number)
% - r: radius (number)
% - c: contrast number (number)
% - roi_name: name of ROI (string)
% - roi_xyz: optionally specify starting XYZ coordinates. To use this,
% enter 'other' as the ROI name. Otherwise, enter [0 0 0].
% - subjects: cell string of formatted subject numbers (e.g. {'01' '02' '03'})
% - res_dir: directory to contain results
%
...
% roi_picker('IEHFMRI',.001,5,9,1,'RTPJ','[0 0 0]',{'104' '105' '107'},'results_normed');
% Written by Alek Chakroff, November 2009/Emily Wasserman, December 2015
...
temp=subjects;
subjects={};
for thissub=1:length(temp)
    subjects{end+1}=fullfile(root_dir,study,[subj_tag '_' temp{thissub}]);
end
clear temp;
