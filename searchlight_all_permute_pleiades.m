function searchlight_all_permute_pleiades(rootdir,study,subj_tag,resdir,sub_nums,conditions,sph,B_in,outtag,permNum)
...
% - permNum: array of indices of regressors to permute. E.g., [2] to permute only the second matrix.
%
% Output:
	...
	for subj=1:length(subjIDs) %grabbing beta images
	    ...
	    for i = 1:length(greymattermask2)% for each voxel
	        ...
	        if length(goodrows) > 9 % if there are at least 10 good voxels in this sphere
	            ...
	            % % permute the neural similarity matrix:
	            % shuffle=randperm(conditions);
	            % simmat		= simmat(shuffle,shuffle);
